...
inline TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::CELL_TYPE::CELL_TYPE() : m_child(NULL)
{
} // constructor
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
inline unsigned TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::CELL_TYPE::ComputeChild(const POINT_TYPE
                                                                                          &item) const
{
    ASSERT(!IsLeaf());
    unsigned idx = 0;
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
inline typename TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::POINT_TYPE TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::
    CELL_TYPE::ComputeChildCenter(const POINT_TYPE &center, TYPE radius, unsigned idxChild)
{
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename Functor>
inline TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::TOctree(const ITEMARR_TYPE &items,
...
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::_Insert(CELL_TYPE &cell,
                                                           const POINT_TYPE &center,
                                                           TYPE radius,
                                                           IDX_TYPE start,
                                                           IDX_TYPE size,
                                                           _InsertData<Functor> &insertData)
{
    ASSERT(size > 0);
    ...
    if (bForceSplit || insertData.split(size, radius))
    {
        ...
        struct ChildData
        {
            ...
            {
                ESTART = 0,
                EEND   = CELL_TYPE::numChildren,
                ESIZE  = CELL_TYPE::numChildren * 2,
                EALL   = CELL_TYPE::numChildren * 3
            };
            ...
            ChildData()
            {
                memset(data, 0, sizeof(IDX_TYPE) * EALL);
            }

            inline IDX_TYPE Start(unsigned i) const
            {
                return data[ESTART + i];
            }

            inline IDX_TYPE &Start(unsigned i)
            {
                return data[ESTART + i];
            }

            inline IDX_TYPE End(unsigned i) const
            {
                return data[EEND + i];
            }

            inline IDX_TYPE &End(unsigned i)
            {
                return data[EEND + i];
            }

            inline IDX_TYPE Size(unsigned i) const
            {
                return data[ESIZE + i];
            }

            inline IDX_TYPE &Size(unsigned i)
            {
                return data[ESIZE + i];
            }
        } childD;
    ...
    else
    {
        ...
        for (IDX_TYPE idx = start; idx != _InsertData<Functor>::NO_INDEX;
             idx          = insertData.successors[idx])
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename Functor>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::Insert(const ITEMARR_TYPE &items,
                                                                 const AABB_TYPE &aabb,
                                                                 Functor split)
{
    Release();
...
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::CollectCells(const CELL_TYPE &cell,
...
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::_ParseCells(CELL_TYPE &cell,
                                                               TYPE radius,
                                                               PARSER &parser)
{
    if (cell.IsLeaf())
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename INSERTER>
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::_Collect(const CELL_TYPE &cell,
                                                            const AABB_TYPE &aabb,
                                                            INSERTER &inserter) const
{
    if (cell.IsLeaf())
                                                            ...
                                                            INSERTER &inserter) const
{
    ...
    for (int i = 0; i < CELL_TYPE::numChildren; ++i)
    {
        ...
        if (childCell.IsLeaf())
        {
            if (collector.Intersects(CELL_TYPE::ComputeChildCenter(cell.GetCenter(), childRadius, i),
                                     childRadius))
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename INSERTER>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::Collect(INSERTER &inserter,
                                                                  const AABB_TYPE &aabb) const
{
    _Collect(m_root, aabb, inserter);
}

template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::Collect(IDXARR_TYPE &indices,
                                                                  const AABB_TYPE &aabb) const
{
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::Collect(IDX_TYPE maxNeighbors,
                                                                  IDXARR_TYPE &indices,
                                                                  const AABB_TYPE &aabb) const
{
...
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::_Traverse(const CELL_TYPE &cell,
                                                             TYPE radius,
                                                             const TFrustum<FTYPE, FDIMS> &frustum,
                                                             INSERTER &inserter) const
{
    ASSERT(!cell.IsLeaf());
    ...
    case CLIPPED:
    {
        ...
        for (int i = 0; i < CELL_TYPE::numChildren; ++i)
        {
            ...
            if (childCell.IsLeaf())
            {
                const AABB_TYPE childAabb(CELL_TYPE::ComputeChildCenter(cell.GetCenter(),
                                                                        childRadius,
                                                                        i),
                                          childRadius);
...
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::_TraverseCells(CELL_TYPE &cell,
                                                                  TYPE radius,
                                                                  const TFrustum<FTYPE, FDIMS> &frustum,
                                                                  PARSER &parser)
{
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename FTYPE, int FDIMS, typename INSERTER>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::Traverse(const TFrustum<FTYPE, FDIMS> &frustum,
                                                                   INSERTER &inserter) const
{
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename FTYPE, int FDIMS, typename PARSER>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::TraverseCells(const TFrustum<FTYPE, FDIMS> &frustum,
                                                                        PARSER &parser)
{
...
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::_SplitVolume(const CELL_TYPE &parentCell,
                                                                TYPE parentRadius,
                                                                unsigned idxChild,
                                                                float maxArea,
                                                                ...
                                                                const UnsignedArr &indices)
{
    ...
    struct GenerateSamples
    {
        ...
        GenerateSamples(const UnsignedArr &_indices)
            : indices(_indices), numSamples((unsigned)indices.size()), halfSamples(numSamples / 2),
              numCommonAxis(halfSamples == 4 ? 1 : 2), arrHalfIndices(0, numSamples)
        {
    ...
    if (bestSplit != NO_ID)
    {
        ...
        if (bestAs[0] < maxArea)
            ...
            _SplitVolume(parentCell,
                         parentRadius,
                         idxChild,
                         maxArea,
                         areaEstimator,
                         chunkInserter,
                         pairHalfIndices.first);
        if (bestAs[1] < maxArea)
    ...
    if (halfIndices.front().first.size() == 4)
    {
        ...
        {
            const cList<PairIndices> qIndicesFirst(std::move(GenerateSamples(pairHalfIndices.first)
                                                                 .arrHalfIndices));
            const cList<PairIndices> qIndicesSecond(std::move(GenerateSamples(pairHalfIndices.second)
                                                                  .arrHalfIndices));
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename AREAESTIMATOR, typename CHUNKINSERTER>
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::SplitVolume(float maxArea,
                                                               AREAESTIMATOR &areaEstimator,
                                                               CHUNKINSERTER &chunkInserter)
{
    CELL_TYPE parent;
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::_GetDebugInfo(const CELL_TYPE &cell,
                                                                 unsigned nDepth,
                                                                 DEBUGINFO &info) const
{
    if (cell.IsLeaf())
    {
...
{
    // VERBOSE("NoItems: %d; Mem %s; MemItems %s; MemStruct %s; AvgMemStruct %.2f%%%%; NoNodes %d;
    // NoLeaf %d; AvgLeaf %.2f%%%%; AvgDepth %.2f; MinDepth %d; MaxDepth %d",
    VERBOSE("NumItems %d; Mem %s (%s items, %s struct - %.2f%%%%); NumNodes %d (leaves %d - "
            "%.2f%%%%); Depth %.2f (%d min, %d max)",
            info.numItems,
            Util::formatBytes(info.memSize).c_str(),
            Util::formatBytes(info.memItems).c_str(),
            Util::formatBytes(info.memStruct).c_str(),
            double(info.memStruct) * 100.0 / info.memSize,
            info.numNodes,
            info.numLeaves,
            float(info.numLeaves * 100) / info.numNodes,
            info.avgDepth,
            info.minDepth,
            info.maxDepth);
} // LogDebugInfo
...
inline bool OctreeTest(unsigned iters, unsigned maxItems = 1000, bool bRandom = true)
{
    ...
    for (unsigned iter = 0; iter < iters; ++iter)
    {
        ...
        TestTree tree(items,
                      aabb,
                      [](typename TestTree::IDX_TYPE size, typename TestTree::Type radius)
                      {
    ...
    VERBOSE("Test %s (TotalMissed %d, TotalExtra %d)",
            (nTotalMissed == 0 && nTotalExtra == 0 ? "successful" : "FAILED"),
            nTotalMissed,
            nTotalExtra);
#endif
