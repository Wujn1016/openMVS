...
template <typename TYPE> class TFlags
{
public:
    typedef TYPE Type;
...
public:
    ...
    inline bool isSet(Type aFlag) const
    {
        return (flags & aFlag) == aFlag;
    }

    inline bool isSet(Type aFlag, Type nF) const
    {
        return (flags & (aFlag | nF)) == aFlag;
    }

    inline bool isSetExclusive(Type aFlag) const
    {
        return flags == aFlag;
    }

    inline bool isAnySet(Type aFlag) const
    {
    ...
    inline bool isAnySetExclusive(Type aFlag) const
    {
        return (flags & aFlag) != 0 && (flags & ~aFlag) == 0;
    }
    ...
    inline void set(Type aFlag)
    {
        flags |= aFlag;
    }

    inline void unset(Type aFlag)
    {
        flags &= ~aFlag;
    }

    inline void flip(Type aFlag)
    {
        flags ^= aFlag;
    }

    inline void operator=(TFlags rhs)
    {
        flags = rhs.flags;
    }
...
template <typename TYPE> class THistogram
{
public:
    typedef TYPE Type;
...
public:
    ...
    THistogram(const std::pair<Type, Type> &range, size_t bins = 10)
        : Start(range.first), End(range.second), BinInterval(Type(bins) / (End - Start)),
          Freq(bins, 0), Overflow(0), Underflow(0)
    {
    }

    // Construct a histogram from a sequence of data
    template <typename DataInputIterator> void Add(DataInputIterator begin, DataInputIterator end)
    {
    ...
    inline size_t GetOverflow() const
    {
        return Overflow;
    }
    ...
    // Get frequencies
    inline const std::vector<size_t> &GetHist() const
    {
        return Freq;
    }
    ...
    // Get start
    inline Type GetStart() const
    {
        return Start;
    }

    // Get End
    inline Type GetEnd() const
    {
        return End;
    }
    ...
    Type GetApproximatePermille(float permille) const
    {
        ...
        // 当(Num) / NumValues >permille（0.95）是返回UpperBound
        // 相当于返回的是0.95-1的区域
...
public:
    ...
    static String getUniqueName(TCHAR dash = '-')
    {
        ...
        LPTSTR szTime        = szDate + 1 + strftime(szDate, 80, "%y%m%d", tmp);
    ...
    static String getRelativePath(const String &currentPath, const String &targetPath)
    {
        ...
        while (idxCurrentPath < currentPathValues.size() && idxTargetPath < targetPathValues.size() &&
#ifdef _MSC_VER
...
// returns the path shared by the given paths
    ...
    static String formatBytes(int64_t aBytes)
    {
        ...
        {
            return String::FormatString("%.02fTB",
    ...
    char *to_next;
    if (converter.out(state, ws.data(), ws.data() + ws.length(), from_next, &to[0], &to[0] + to.size(), to_next)
        != converter_type::ok)
        return String();
    ...
    struct Progress
    {
        ...
        Progress(const String &_msg, size_t _total, Timer::Type _slp = 100 /*ms*/)
            : msg(_msg), total(_total), slp(_slp), start(Timer::GetSysTime()), lastElapsed(0),
              lastMsgLen(0), processed(0)
        {
        }
        ...
        void process()
        {
            ...
            const Timer::Type remaining(percentage < 0.01f && (done < 10 || elapsed < 10 * 1000)
                                            ? Timer::Type(0)
            ...
            print(String::FormatString(_T("%s %u (%.2f%%, %s, ETA %s)..."),
                                       ...
                                       percentage * 100.f,
                                       formatTime((int64_t)elapsed, 1).c_str(),
                                       formatTime((int64_t)remaining, 2).c_str()));
        }
        ...
        void close()
        {
            ...
            const Timer::Type elapsed(Timer::SysTime2TimeMs(Timer::GetSysTime() - start));
            // display progress
            print(String::FormatString(_T("%s %u (100%%, %s)"),
                                       msg.c_str(),
                                       total,
                                       formatTime((int64_t)elapsed).c_str()));
            std::cout << _T("\n");
            processed = 0;
        }
