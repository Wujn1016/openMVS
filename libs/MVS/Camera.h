...
//   P = KR[I|-C]
// where R and C represent the camera orientation and position relative to the world coordinate
// system; R is expressed as the rotation from world to camera coordinates C is expressed as the
// explicit camera center in world coordinates (as opposite to standard form t which is negated and
// has the rotation of the camera already applied P = K[R|t]); the world and camera coordinates
// system is right handed, with x pointing right, y pointing down, and z pointing forward (see: R.
// Hartley, "Multiple View Geometry," 2004, pp. 156.); the projection in image coordinates uses the
// convention that the center of a pixel is defined at integer coordinates, i.e. the center is at
// (0, 0) and the top left corner is at (-0.5, -0.5)
...
public:
    ...
    inline CameraIntern(const Matrix3x3 &_K, const Matrix3x3 &_R, const Point3 &_C)
        : K(_K), R(_R), C(_C)
    {
    }

    inline void SetT(const CMatrix &T)
    {
        C = R.t() * (-T);
    }

    inline CMatrix GetT() const
    {
        return R * (-C);
    }

    // returns the camera's view forward direction
    inline Point3 Direction() const
    {
        return R.row(2); /* equivalent to R.t() * Vec(0,0,1) */
    }

    // returns the camera's view up direction
    inline Point3 UpDirection() const
    {
        return -R.row(1); /* equivalent to R.t() * Vec(0,-1,0) */
    }
    ...
    inline REAL GetFocalLengthRatio() const
    {
        return (K(1, 1) / K(0, 0));
    }

    // returns the principal-point
    inline Point2 GetPrincipalPoint() const
    {
    ...
    static inline TMatrix<TYPE, 3, 3> ComposeK(const TYPE &fX,
                                               const TYPE &fY,
                                               TYPER w = TYPER(1),
                                               TYPER h = TYPER(1))
    {
    ...
    template <typename TYPE, typename TYPER>
    static inline TMatrix<TYPE, 3, 3> ComposeInvK(const TYPE &fX,
                                                  const TYPE &fY,
                                                  TYPER w = TYPER(1),
                                                  TYPER h = TYPER(1))
    {
    ...
    {
        return TPoint2<TYPE>((x.x + TYPE(0.5)) * s - TYPE(0.5), (x.y + TYPE(0.5)) * s - TYPE(0.5));
    }
    ...
    {
        return TMatrix<TYPE, 3, 3>(K(0, 0) * s,
                                   K(0, 1) * s,
                                   (K(0, 2) + TYPE(0.5)) * s - TYPE(0.5),
                                   TYPE(0),
                                   K(1, 1) * s,
                                   (K(1, 2) + TYPE(0.5)) * s - TYPE(0.5),
                                   TYPE(0),
                                   TYPE(0),
                                   TYPE(1));
    }
    ...
    template <typename TYPE>
    static inline TMatrix<TYPE, 3, 3> ScaleK(const TMatrix<TYPE, 3, 3> &K,
                                             const cv::Size &size,
                                             ...
                                             bool keepAspect = false)
    {
        ...
        return TMatrix<TYPE, 3, 3>(K(0, 0) * s.x,
                                   K(0, 1) * s.x,
                                   (K(0, 2) + TYPE(0.5)) * s.x - TYPE(0.5),
                                   TYPE(0),
                                   K(1, 1) * s.y,
                                   (K(1, 2) + TYPE(0.5)) * s.y - TYPE(0.5),
                                   TYPE(0),
                                   TYPE(0),
                                   TYPE(1));
    }
    ...
    template <typename TYPE> static inline TMatrix<TYPE, 3, 3> InvK(const TMatrix<TYPE, 3, 3> &K)
    {
        ASSERT(ISZERO(K(0, 1)));
        TMatrix<TYPE, 3, 3> invK(TMatrix<TYPE, 3, 3>::IDENTITY);
    ...
    template <typename TYPE> inline TMatrix<TYPE, 3, 3> GetK(uint32_t width, uint32_t height) const
    {
        ASSERT(width > 0 && height > 0);
        ...
        ASSERT(ISZERO(K(0, 1)));
        return ComposeK(TYPE(K(0, 0) * scale), TYPE(K(1, 1) * scale), width, height);
    }
    ...
    inline TMatrix<TYPE, 3, 3> GetInvK(uint32_t width, uint32_t height) const
    {
        ...
        ASSERT(ISZERO(K(0, 1)));
        return ComposeInvK(TYPE(K(0, 0) * scale), TYPE(K(1, 1) * scale), width, height);
    }

    // return the OpenGL projection matrix corresponding to K:
    // - flip: if true, flip the y axis to match OpenGL image convention
    template <typename TYPE>
    static inline TMatrix<TYPE, 4, 4> ProjectionMatrixOpenGL(const TMatrix<TYPE, 3, 3> &K,
                                                             const cv::Size &size,
                                                             TYPE nearZ,
                                                             TYPE farZ,
                                                             bool flip = true)
    {
        ...
        return TMatrix<TYPE, 4, 4>(fx * ihw,
                                   skew * ihw,
                                   cx * ihw - TYPE(1),
                                   0,
                                   0,
    ...
    inline Matrix4x4 GetProjectionMatrixOpenGL(const cv::Size &size,
                                               REAL nearZ,
                                               REAL farZ,
...
public:
    ...
    void Transform(const Matrix3x3 &R,
                   const Point3 &t,
                   const REAL &s); // transform camera by the given similarity transform

    REAL PointDepth(const Point3 &X)
        const; // computes the depth of the given 3D point seen from this camera
    ...
    inline REAL Distance(const Point3 &X) const
    {
        return SQRT(DistanceSq(X));
    }

    static REAL StereoRectify(const cv::Size &size1,
                              ...
                              Matrix3x3 &K1,
                              Matrix3x3 &K2);
    static REAL StereoRectifyFusiello(const cv::Size &size1,
                                      const Camera &camera1,
                                      const cv::Size &size2,
                                      const Camera &camera2,
                                      Matrix3x3 &R1,
                                      ...
                                      Matrix3x3 &K1,
                                      Matrix3x3 &K2);
    static void SetStereoRectificationROI(const Point3fArr &points1,
                                          cv::Size &size1,
                                          const Camera &camera1,
                                          const Point3fArr &points2,
                                          cv::Size &size2,
                                          const Camera &camera2,
                                          const Matrix3x3 &R1,
                                          const Matrix3x3 &R2,
    ...
    template <typename TYPE> inline TPoint3<TYPE> ProjectPointRT3(const TPoint3<TYPE> &X) const
    {
        return R * (X - C);
    }

    template <typename TYPE> inline TPoint2<TYPE> ProjectPointRT(const TPoint3<TYPE> &X) const
    {
    ...
    template <typename TYPE> inline TPoint2<TYPE> ProjectPoint(const TPoint3<TYPE> &X) const
    {
    ...
    template <typename TYPE> inline TPoint3<TYPE> ProjectPointP3(const TPoint3<TYPE> &X) const
    {
        const REAL *const p(P.val);
        return TPoint3<TYPE>((TYPE)(p[0 * 4 + 0] * X.x + p[0 * 4 + 1] * X.y + p[0 * 4 + 2] * X.z
                                    + p[0 * 4 + 3]),
                             (TYPE)(p[1 * 4 + 0] * X.x + p[1 * 4 + 1] * X.y + p[1 * 4 + 2] * X.z
                                    + p[1 * 4 + 3]),
                             (TYPE)(p[2 * 4 + 0] * X.x + p[2 * 4 + 1] * X.y + p[2 * 4 + 2] * X.z
                                    + p[2 * 4 + 3]));
    }

    template <typename TYPE> inline TPoint2<TYPE> ProjectPointP(const TPoint3<TYPE> &X) const
    {
    ...
    template <typename TYPE> inline TPoint2<TYPE> TransformPointI2V(const TPoint2<TYPE> &x) const
    {
        return TPoint2<TYPE>(TYPE(x.x - K(0, 2)), TYPE(x.y - K(1, 2)));
    }

    // un-project from image pixel coords to the camera space (z=1 plane by default)
    template <typename TYPE> inline TPoint3<TYPE> TransformPointI2C(const TPoint2<TYPE> &x) const
    {
        return TPoint3<TYPE>(TYPE((x.x - K(0, 2)) / K(0, 0)),
                             TYPE((x.y - K(1, 2)) / K(1, 1)),
                             TYPE(1));
    }

    // un-project from image pixel coords to the camera space
    template <typename TYPE> inline TPoint3<TYPE> TransformPointI2C(const TPoint3<TYPE> &X) const
    {
        return TPoint3<TYPE>(TYPE((X.x - K(0, 2)) * X.z / K(0, 0)),
                             TYPE((X.y - K(1, 2)) * X.z / K(1, 1)),
                             X.z);
    }

    template <typename TYPE> inline TPoint3<TYPE> TransformPointC2W(const TPoint3<TYPE> &X) const
    {
        return R.t() * X + C;
    }

    template <typename TYPE> inline TPoint3<TYPE> TransformPointI2W(const TPoint2<TYPE> &x) const
    {
        return TransformPointC2W(TransformPointI2C(x));
    }

    template <typename TYPE> inline TPoint3<TYPE> TransformPointI2W(const TPoint3<TYPE> &X) const
    {
        return TransformPointC2W(TransformPointI2C(X));
    }

    template <typename TYPE> inline TPoint3<TYPE> RayPoint(const TPoint2<TYPE> &x) const
    {
        return R.t() * TransformPointI2C(x);
    }

    template <typename TYPE> inline TPoint3<TYPE> RayPointP(const TPoint2<TYPE> &x) const
    {
        TPoint3<TYPE> ray;
    ...
    template <typename TYPE> inline TPoint2<TYPE> TransformPointC2I(const TPoint2<TYPE> &x) const
    {
        return TPoint2<TYPE>(TYPE(K(0, 2) + K(0, 0) * x.x), TYPE(K(1, 2) + K(1, 1) * x.y));
    }

    // project from the camera space to view plane
    template <typename TYPE> inline TPoint2<TYPE> TransformPointC2V(const TPoint3<TYPE> &X) const
    {
        return TPoint2<TYPE>(TYPE(K(0, 0) * X.x / X.z), TYPE(K(1, 1) * X.y / X.z));
    }

    // project from the camera space to image pixels
    template <typename TYPE> inline TPoint2<TYPE> TransformPointC2I(const TPoint3<TYPE> &X) const
    {
        return TransformPointC2I(TPoint2<TYPE>(X.x / X.z, X.y / X.z));
    }

    template <typename TYPE> inline TPoint3<TYPE> TransformPointW2C(const TPoint3<TYPE> &X) const
    {
        return R * (X - C);
    }

    template <typename TYPE> inline TPoint2<TYPE> TransformPointW2I(const TPoint3<TYPE> &X) const
    {
        return TransformPointC2I(TransformPointW2C(X));
    }

    template <typename TYPE> inline TPoint3<TYPE> TransformPointW2I3(const TPoint3<TYPE> &X) const
    {
    ...
    {
        return TPoint3<TYPE>(TYPE((x.x - K(0, 2)) / K(0, 0)), TYPE((x.y - K(1, 2)) / K(1, 1)), x.z);
    }
    ...
    template <typename TYPE> inline TYPE GetFootprintImage(const TPoint3<TYPE> &X) const
    {
#if 0
    ...
    template <typename TYPE> inline TYPE GetFootprintWorld(const TPoint2<TYPE> &x, TYPE depth) const
    {
        return depth / SQRT(SQUARE(GetFocalLength()) + normSq(TransformPointI2V(x)));
    }

    // same as above, but the 3D point is given
    template <typename TYPE> inline TYPE GetFootprintWorldSq(const TPoint3<TYPE> &X) const
    {
        const TPoint3<TYPE> camX(TransformPointW2C(X));
        return GetFootprintWorldSq(TPoint2<TYPE>(camX.x / camX.z, camX.y / camX.z), camX.z);
    }

    template <typename TYPE> inline TYPE GetFootprintWorld(const TPoint3<TYPE> &X) const
    {
        const TPoint3<TYPE> camX(TransformPointW2C(X));
