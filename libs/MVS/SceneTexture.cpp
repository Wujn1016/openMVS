...
#if !defined(EIGEN_DEFAULT_TO_ROW_MAJOR) || EIGEN_WORLD_VERSION > 3 \
...
LBPInference::EnergyType STCALL SmoothnessPotts(LBPInference::NodeID,
                                                LBPInference::NodeID,
                                                LBPInference::LabelID l1,
                                                LBPInference::LabelID l2)
{
    return l1 == l2 && l1 != 0 && l2 != 0 ? LBPInference::EnergyType(0)
                                          : LBPInference::EnergyType(LBPInference::MaxEnergy);
}
} // namespace MVS
#endif
...
struct MeshTexture
{
    ...
    struct RasterMesh : TRasterMesh<RasterMesh>
    {
        ...
        RasterMesh(const Mesh::VertexArr &_vertices,
    ...
    typedef cList<FaceData, const FaceData &, 0, 8, uint32_t>
        FaceDataArr; // store information about one face seen from several views
    typedef cList<FaceDataArr, const FaceDataArr &, 2, 1024, FIndex>
        FaceDataViewArr; // store data for all the faces of the mesh

    typedef cList<Mesh::FaceIdxArr, const Mesh::FaceIdxArr &, 2, 1024, FIndex>
        VirtualFaceIdxsArr; // store face indices for each virtual face
    ...
    struct VertexPatchIterator
    {
        ...
        inline VertexPatchIterator(const PatchIndex &patchIndex, const SeamVertices &seamVertices)
            : idx(NO_ID)
        {
...
public:
    ...
    bool ListCameraFaces(FaceDataViewArr &,
    ...
    void CreateVirtualFaces(const FaceDataViewArr &facesDatas,
                            FaceDataViewArr &virtualFacesDatas,
                            VirtualFaceIdxsArr &virtualFaces,
                            unsigned minCommonCameras  = 2,
                            float thMaxNormalDeviation = 25.f) const;
    IIndexArr SelectBestView(const FaceDataArr &faceDatas,
                             FIndex fid,
                             unsigned minCommonCameras,
                             float ratioAngleToQuality) const;

    bool FaceViewSelection(unsigned minCommonCameras,
                           float fOutlierThreshold,
    ...
    template <typename PIXEL> static inline PIXEL RGB2YCBCR(const PIXEL &v)
    {
        typedef typename PIXEL::Type T;
        return PIXEL(v[0] * T(0.299) + v[1] * T(0.587) + v[2] * T(0.114),
                     v[0] * T(-0.168736) + v[1] * T(-0.331264) + v[2] * T(0.5) + T(128),
                     v[0] * T(0.5) + v[1] * T(-0.418688) + v[2] * T(-0.081312) + T(128));
    }

    template <typename PIXEL> static inline PIXEL YCBCR2RGB(const PIXEL &v)
    {
        typedef typename PIXEL::Type T;
        const T v1(v[1] - T(128));
        const T v2(v[2] - T(128));
        return PIXEL(v[0] /* * T(1) + v1 * T(0)*/ + v2 * T(1.402),
...
protected:
    ...
    static void PoissonBlending(const Image32F3 &src,
                                Image32F3 &dst,
                                const Image8U &mask,
                                float bias = 1.f);
...
public:
    ...
    Mesh::FaceFacesArr
        &faceFaces; // for each face, the list of adjacent faces, NO_ID for border edges (optional)
...
static Image8U DetectInvalidImageRegions(const Image8U3 &image)
{
    ...
    if (imageGray(image.rows / 2, image.cols - 1) == 0)
        cv::floodFill(imageGray,
                      mask,
                      cv::Point(image.cols - 1, image.rows / 2),
                      255,
                      NULL,
                      cv::Scalar(0),
                      upDiff,
...
MeshTexture::MeshTexture(Scene &_scene, unsigned _nResolutionLevel, unsigned _nMinResolution)
    : nResolutionLevel(_nResolutionLevel), nMinResolution(_nMinResolution),
      vertexFaces(_scene.mesh.vertexFaces), vertexBoundary(_scene.mesh.vertexBoundary),
      faceFaces(_scene.mesh.faceFaces), faceTexcoords(_scene.mesh.faceTexcoords),
      faceTexindices(_scene.mesh.faceTexindices), texturesDiffuse(_scene.mesh.texturesDiffuse),
      vertices(_scene.mesh.vertices), faces(_scene.mesh.faces), images(_scene.images), scene(_scene)
{
}
...
bool MeshTexture::ListCameraFaces(FaceDataViewArr &facesDatas,
                                  float fOutlierThreshold,
                                  int nIgnoreMaskLabel,
                                  const IIndexArr &_views)
{
    // create faces octree
    Mesh::Octree octree;
    ...
    for (IIndex idxView : views)
    {
        ...
        grad[0] = cv::Mat(imageGradMag.rows,
                          imageGradMag.cols,
                          cv::DataType<real>::type,
                          (void *)mGrad[0].data());
        mGrad[1].resize(imageGradMag.rows, imageGradMag.cols);
        grad[1] = cv::Mat(imageGradMag.rows,
                          imageGradMag.cols,
                          cv::DataType<real>::type,
                          (void *)mGrad[1].data());
#if 1
    ...
    cv::filter2D(imageGradMag, grad[1], cv::DataType<real>::type, kernel.t());
#endif
        (TImage<real>::EMatMap)
            imageGradMag = (mGrad[0].cwiseAbs2() + mGrad[1].cwiseAbs2()).cwiseSqrt();
        ...
        const TFrustum<float, 5> frustum(Matrix3x4f(imageData.camera.P),
                                         (float)imageData.width,
        ...
        if (nIgnoreMaskLabel >= 0)
        {
            // import mask
            BitMatrix bmask;
            DepthEstimator::ImportIgnoreMask(imageData,
                                             imageData.GetSize(),
                                             (uint16_t)OPTDENSE::nIgnoreMaskLabel,
                                             bmask,
                                             &rasterer.mask);
        }
...
// compute the projection area of visible faces
    ...
    #pragma omp critical
#endif
        {
            ...
            // + sharpness: sharper image or image resolution or how close is to the face will result
            // in higher gradient on the same face
...
// order the camera view scores with highest score first and return the list of first
// <minCommonCameras> cameras ratioAngleToQuality represents the ratio in witch we combine normal
// angle to quality for a face to obtain the selection score
...
IIndexArr MeshTexture::SelectBestView(const FaceDataArr &faceDatas,
                                      FIndex fid,
                                      unsigned minCommonCameras,
                                      float ratioAngleToQuality) const
{
    ...
    FOREACH(idxFaceData, faceDatas)
    {
        ...
        scores[idxFaceData] = ratioAngleToQuality * cosFaceCam
                              + (1.f - ratioAngleToQuality) * faceData.quality / maxQuality;
    }
    ...
    anglePodium.Sort(
        [&cameraAngles](IIndex i, IIndex j)
        {
...
void MeshTexture::CreateVirtualFaces(const FaceDataViewArr &facesDatas,
                                     FaceDataViewArr &virtualFacesDatas,
                                     VirtualFaceIdxsArr &virtualFaces,
                                     unsigned minCommonCameras,
                                     float thMaxNormalDeviation) const
{
    ...
    do
    {
        ...
        {
            const IIndexArr selectedCams = SelectBestView(centerFaceDatas,
                                                          virtualFaceCenterFaceID,
                                                          minCommonCameras,
                                                          ratioAngleToQuality);
...
inline T MultiGaussUnnormalized(const Eigen::Matrix<T, N, 1> &centered,
                                const Eigen::Matrix<T, N, N> &covarianceInv)
{
    return EXP(T(-0.5) * T(centered.adjoint() * covarianceInv * centered));
}

template <typename T, int N>
inline T MultiGaussUnnormalized(const Eigen::Matrix<T, N, 1> &X,
                                const Eigen::Matrix<T, N, 1> &mu,
                                const Eigen::Matrix<T, N, N> &covarianceInv)
{
...
bool MeshTexture::FaceViewSelection(unsigned minCommonCameras,
                                    float fOutlierThreshold,
                                    float fRatioDataSmoothness,
                                    int nIgnoreMaskLabel,
                                    const IIndexArr &views)
{
    ...
    // create texture patches
    {
        ...
        // create faces graph,图类型，边迭代器，出边迭代器
        ...
        // create the graph of faces: each vertex is a face and the edges are the edges shared by
        // the faces
        // 每个三角面在图中创建一个顶点，确保顶点序号与面序号相同
        FOREACH(idxFace, faces)
        {
            MAYBEUNUSED const Mesh::FIndex idx((Mesh::FIndex)boost::add_vertex(graph));
            ASSERT(idx == idxFace);
        }
        // 遍历所有的面，找出面三条边所邻接的面，如果两个顶点所对应的面是邻接面，创建边
        // 如果两个邻接面，其中一个面是空的，放入seamEdges中
        FOREACH(idxFace, faces)
        {
        ...
        // 从单独face中开始创建face群path
        {
            ...
            labels.resize(faces.size());
            {
                ...
                const LBPInference::EnergyType MaxEnergy(fRatioDataSmoothness
                                                         * (LBPInference::EnergyType)
                                                             LBPInference::MaxEnergy);
    ...
    // assign the optimal view (label) to each face
        ...
        // create texture patches
        {
            ...
            for (const PairIdx *pEdge    = seamEdges.Begin() + startLabelSeamEdges,
                               *pEdgeEnd = seamEdges.End();
                 pEdge != pEdgeEnd;
                 ++pEdge)
...
void MeshTexture::CreateSeamVertices()
{
    ...
    // each patch contains the list of edges belonging to that texture patch, starting from that
    // vertex (usually there are pairs of edges in each patch, representing the two edges starting
    // from that vertex separating two valid patches)
    VIndex vs[2];
...
void MeshTexture::GlobalSeamLeveling()
{
    ...
    Lhs.prune(
        [](const int &row, const int &col, const float &) -> bool
        {
            return col <= row;
        });
    ...
    #pragma omp parallel for
        ...
        for (int channel = 0; channel < 3; ++channel)
        {
            ...
            const Eigen::Map<Eigen::VectorXf, Eigen::Unaligned, Eigen::Stride<0, 3>>
                b(coeffB.front().ptr() + channel, rowsA);
            ...
            Eigen::Map<Eigen::VectorXf, Eigen::Unaligned, Eigen::Stride<0, 3>>(colorAdjustments.data()
                                                                                   + channel,
                                                                               rowsX) = x.array()
                                                                                        - x.mean();
            DEBUG_LEVEL(3,
                        "\tcolor channel %d: %d iterations, %g residual",
                        channel,
                        solver.iterations(),
                        solver.error());
        }
...
#undef ERODEDIR
    ...
    for (int r = 0; r < mask.rows; ++r)
    {
        for (int c = 0; c < mask.cols; ++c)
        {
            ...
            if (v != interior)
                continue;
            if (mask(r - 1, c) == empty || mask(r, c - 1) == empty || mask(r + 1, c) == empty
                || mask(r, c + 1) == empty)
                v = border;
        }
    }
        ...
        for (int x = 0; x < width; ++x)
        {
            ...
            for (int j = -1; j <= 1; ++j)
            {
                for (int i = -1; i <= 1; ++i)
                {
                    ...
                    if (ISINSIDE(xn, 0, width) && ISINSIDE(yn, 0, height) && ISEMPTY(mask, xn, yn))
                    {
            ...
            for (PixelVector::const_iterator it = emptyPixels.cbegin(); it != emptyPixels.cend(); ++it)
            {
                for (int j = -1; j <= 1; j++)
                {
                    for (int i = -1; i <= 1; i++)
                    {
                        ...
                        if (ISINSIDE(xn, 0, width) && ISINSIDE(yn, 0, height)
...
void MeshTexture::PoissonBlending(const Image32F3 &src, Image32F3 &dst, const Image8U &mask, float bias)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        case interior:
        {
            ...
            coeffB[idxCenter] = (bias == 1.f ? ColorLaplacian(src, i)
                                             : ColorLaplacian(src, i) * bias
                                                   + ColorLaplacian(dst, i) * (1.f - bias));
        }
        break;
        }
    }
    ...
    for (int channel = 0; channel < 3; ++channel)
    {
        const Eigen::Map<Eigen::VectorXf, Eigen::Unaligned, Eigen::Stride<0, 3>> b(coeffB.front().ptr()
                                                                                       + channel,
                                                                                   nnz);
            ...
            for (const SeamVertex::Patch::Edge &edge0 : patch0.edges)
            {
                ...
                FOREACH(idxVertPatch1, seamVertex0.patches)
                {
                    ...
                    struct RasterPatch
                    {
                        ...
                        inline RasterPatch(Image32F3 &_image,
                                           Image8U &_mask,
                            ...
                            : image(_image), mask(_mask), image0(_image0), image1(_image1), p0(_p0),
                              p0Dir(_p0Adj - _p0), p1(_p1), p1Dir(_p1Adj - _p1),
                              length((float)norm(p0Dir)), sampler()
                        {
                        }
                        ...
                        inline void operator()(const ImageRef &pt)
                        {
                            ...
                            const Color color1(image1.sample<Sampler, Color>(sampler, samplePos1)
                                               / 255.f);
...
// #ifdef TEXOPT_USE_OPENMP
#if 0
    const unsigned numPatches(texturePatches.size() - 1);
    // 找到所有纹理块对应的所有面在纹理块所在的图像中的位置，找到最大AABB包含所有纹理坐标
    // 最终每个顶点所对应的纹理坐标为图像中最大包含矩阵的相对位置，即在该矩形中的坐标
    #pragma omp parallel for schedule(dynamic)
    for (int_t idx = 0; idx < (int_t)numPatches; ++idx)
    {
        TexturePatch &texturePatch = texturePatches[(uint32_t)idx];
#else
    for (TexturePatch *pTexturePatch    = texturePatches.Begin(),
                      *pTexturePatchEnd = texturePatches.End() - 1;
         pTexturePatch < pTexturePatchEnd;
         ++pTexturePatch)
    {
        ...
        // 根据path中的所有面的坐标得出最大包含矩阵，将纹理坐标转化为以矩阵为基准的坐标
    ...
    // create texture
    {
        ...
        {
            if (maxTextureSize > 0
                && (texturePatches[i].rect.width > maxTextureSize
                    || texturePatches[i].rect.height > maxTextureSize))
            {
                DEBUG("error: a patch of size %u x %u does not fit the texture",
                      texturePatches[i].rect.width,
                      texturePatches[i].rect.height);
        ...
        CLISTDEF2IDX(RectsBinPack::RectWIdxArr, TexIndex) placedRects;
        {
            ...
            while (!unplacedRects.empty())
            {
                ...
                if (textureSize == 0)
                {
                    textureSize = RectsBinPack::ComputeTextureSize(unplacedRects,
                ...
                case 0:
                {
                    ...
                    newPlacedRects = pack.Insert(unplacedRects,
                                                 (MaxRectsBinPack::FreeRectChoiceHeuristic)
                                                     typeHeuristic);
                    break;
                }
                case 1:
                {
                    ...
                    newPlacedRects = pack.Insert(unplacedRects,
                                                 (SkylineBinPack::LevelChoiceHeuristic)typeHeuristic);
                    break;
                }
                case 2:
                {
                    ...
                    newPlacedRects = pack.Insert(unplacedRects,
                                                 false,
                                                 (GuillotineBinPack::FreeRectChoiceHeuristic)typeHeuristic,
                                                 (GuillotineBinPack::GuillotineSplitHeuristic)typeSplit);
                    break;
                }
                default:
                    ABORT("error: unknown RectsBinPack type");
                }
                DEBUG_ULTIMATE("\tpacking texture completed: %u initial patches, %u placed "
                               "patches, %u texture-size, %u textures (%s)",
                               texturePatches.size(),
                               newPlacedRects.size(),
                               textureSize,
                               placedRects.size(),
                ...
                if (textureSize == maxTextureSize || unplacedRects.empty())
                {
                    ...
                    texturesDiffuse.emplace_back(textureSize, textureSize)
    ...
    FOREACH(idxTexture, placedRects)
    {
...
#endif
                const TexturePatch
                    &texturePatch = texturePatches[placedRects[idxTexture][idxPlacedPatch].patchIdx];
                const RectsBinPack::Rect &rect = placedRects[idxTexture][idxPlacedPatch].rect;
                // copy patch image
                ASSERT((rect.width == texturePatch.rect.width
                        && rect.height == texturePatch.rect.height)
                       || (rect.height == texturePatch.rect.width
                           && rect.width == texturePatch.rect.height));
                int x(0), y(1);
        ...
        if (fSharpnessWeight > 0)
        {
            ...
            for (auto &textureDiffuse : texturesDiffuse)
            {
                ...
                cv::addWeighted(textureDiffuse,
                                1 + fSharpnessWeight,
                                blurryTextureDiffuse,
                                -fSharpnessWeight,
                                0,
                                textureDiffuse);
            }
...
// texture mesh
//  - minCommonCameras: generate texture patches using virtual faces composed of coplanar triangles
//  sharing at least this number of views (0 - disabled, 3 - good value)
//  - fSharpnessWeight: sharpness weight to be applied on the texture (0 - disabled, 0.5 - good value)
//  - nIgnoreMaskLabel: label value to ignore in the image mask, stored in the MVS scene or next to
//  each image with '.mask.png' extension (-1 - auto estimate mask for lens distortion, -2 - disabled)
bool Scene::TextureMesh(unsigned nResolutionLevel,
                        ...
                        const IIndexArr &views)
{
    ...
    {
        TD_TIMER_STARTD();
        if (!texture.FaceViewSelection(minCommonCameras,
                                       fOutlierThreshold,
                                       fRatioDataSmoothness,
                                       nIgnoreMaskLabel,
                                       views))
            return false;
        DEBUG_EXTRA("Assigning the best view to each face completed: %u faces (%s)",
                    mesh.faces.size(),
                    TD_TIMER_GET_FMT().c_str());
    }
    ...
    // generate the texture image and atlas
    {
        ...
        texture.GenerateTexture(bGlobalSeamLeveling,
                                bLocalSeamLeveling,
                                nTextureSizeMultiple,
                                nRectPackingHeuristic,
                                colEmpty,
                                fSharpnessWeight,
                                maxTextureSize);
        DEBUG_EXTRA("Generating texture atlas and image completed: %u patches, %u image size, %u "
                    "textures (%s)",
                    texture.texturePatches.size(),
                    mesh.texturesDiffuse[0].width(),
                    mesh.texturesDiffuse.size(),
                    TD_TIMER_GET_FMT().c_str());
    }
