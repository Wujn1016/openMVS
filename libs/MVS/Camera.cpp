...
Camera::Camera(const Matrix3x4 &_P, bool bUpdate /*=true*/) : P(_P)
{
    if (bUpdate)
        DecomposeP();
} // Camera

Camera::Camera(const Matrix3x3 &_R, const Point3 &_C, bool bUpdate /*=true*/) : CameraIntern(_R, _C)
{
    if (bUpdate)
        ComposeP_RC();
} // Camera
...
// decomposition of projection matrix into KR[I|-C]: internal calibration ([3,3]), rotation ([3,3])
// and translation ([3,1]) (comparable with OpenCV: normalized cv::decomposeProjectionMatrix)
void MVS::DecomposeProjectionMatrix(const PMatrix &P, KMatrix &K, RMatrix &R, CMatrix &C)
{
    ...
    RQDecomp3x3<REAL>(cv::Mat(3,
                              ...
                              const_cast<REAL *>(P.val))(cv::Rect(0, 0, 3, 3)),
                      K,
                      R);
...
void MVS::AssembleProjectionMatrix(const RMatrix &R, const CMatrix &C, PMatrix &P)
{
    ...
    Eigen::Map<Point3::EVec, 0, Eigen::Stride<0, 4>>(P.val + 3) = ((const Matrix3x3::EMat)R)
                                                                  * (-((const Point3::EVec)C));
} // AssembleProjectionMatrix
...
void GetImagePairROI(const Point3fArr &points1,
                     const Point3fArr &points2,
                     const Matrix3x3 &K1,
                     const Matrix3x3 &K2,
                     const Matrix3x3 &R1,
                     const Matrix3x3 &R2,
                     const Matrix3x3 &invK1,
                     const Matrix3x3 &invK2,
                     AABB2f &roi1h,
                     AABB2f &roi2h)
{
...
void SetCameraMatricesROI(const AABB2f &roi1h,
                          const AABB2f &roi2h,
                          cv::Size &size1,
                          cv::Size &size2,
                          Matrix3x3 &K1,
...
REAL Camera::StereoRectify(const cv::Size &size1,
                           ...
                           Matrix3x3 &K2)
{
    ...
    for (int k = 0; k < 2; k++)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            const Point2f pt((float)((i % 2) * size1.width), (float)((i < 2 ? 0 : 1) * size1.height));
            Point2f ptH;
...
REAL Camera::StereoRectifyFusiello(const cv::Size &size1,
                                   const Camera &camera1,
                                   const cv::Size &size2,
                                   const Camera &camera2,
                                   Matrix3x3 &R1,
...
// adjust rectified camera matrices such that the entire area common to both source images is
// contained in the rectified images; as long as the focal-length on x and the skewness are equal in
// both cameras the point-scale is also equal in both images; note however that the surface-scale
// can still be different (most likely is) as it depends on its (unknown) normal too;
//  - points1 and points2: contain the pairs of corresponding pairs of image projections and their
//  depth
//  - size1 and size2: input the size of the source images, output the size of the rectified images
//  (rectified image sizes are equal)
void Camera::SetStereoRectificationROI(const Point3fArr &points1,
                                       cv::Size &size1,
                                       const Camera &camera1,
                                       const Point3fArr &points2,
                                       cv::Size &size2,
                                       ...
                                       Matrix3x3 &K2)
{
    ...
    // determine the ROIs in rectified images
    AABB2f roi1h, roi2h;
    RECTIFY::GetImagePairROI(points1,
                             points2,
                             K1,
                             K2,
                             R1,
