...
bool Scene::LoadInterface(const String &fileName)
{
    ...
    DEBUG_EXTRA("Scene loaded from interface format (%s):\n"
                ...
                images.size(),
                nCalibratedImages,
                (double)nTotalPixels / (1024.0 * 1024.0),
                (double)nTotalPixels / (1024.0 * 1024.0 * nCalibratedImages),
...
bool Scene::SaveInterface(const String &fileName, int version) const
{
    ...
    FOREACH(i, images)
    {
        ...
        image.viewScores = std::vector<MVS::Interface::Image::ViewScore>(imageData.neighbors.begin(),
                                                                         imageData.neighbors.end());
    ...
    FOREACH(i, pointcloud.points)
    {
        ...
        views.ForEach(
            [&](PointCloud::ViewArr::IDX v)
            {
                ...
                view.confidence                    = (pointcloud.pointWeights.IsEmpty() ? 0.f
                                                                                        : pointcloud.pointWeights[i][v]);
            });
    }
...
bool Scene::LoadDMAP(const String &fileName)
{
    ...
    ViewsMap viewsMap;
    if (!ImportDepthDataRaw(fileName,
                            imageFileName,
                            IDs,
                            imageSize,
                            camera.K,
    ...
    platform.cameras
        .emplace_back(camera.GetScaledK(REAL(1)
                                        / CameraIntern::GetNormalizationScale((uint32_t)imageSize.width,
                                                                              (uint32_t)
                                                                                  imageSize.height)),
                      RMatrix::IDENTITY,
                      CMatrix::ZERO);
    ...
    for (int r = 0; r < depthMap.rows; ++r)
    {
        for (int c = 0; c < depthMap.cols; ++c)
        {
            ...
            if (!normalMap.empty())
                pointcloud.normals
                    .emplace_back(Cast<PointCloud::Normal::Type>(camera.R.t()
                                                                 * Cast<REAL>(normalMap(r, c))));
            if (!confMap.empty())
    ...
    DEBUG_EXTRA("Scene loaded from depth-map format - %dx%d size, %.2f%%%% coverage (%s):\n"
                ...
                depthMap.width(),
                depthMap.height(),
                100.0 * pointcloud.GetSize() / depthMap.area(),
                TD_TIMER_GET_FMT().c_str(),
                (double)image.image.area() / (1024.0 * 1024.0),
                (double)image.image.area() / (1024.0 * 1024.0 * nCalibratedImages),
                pointcloud.GetSize());
    return true;
} // LoadDMAP
...
void Scene::SampleMeshWithVisibility(unsigned maxResolution)
{
    ...
    FOREACH(ID, images)
    {
        ...
        const Image &imageData = images[ID];
        unsigned level(0);
        const unsigned nMaxResolution(Image8U::computeMaxResolution(imageData.width,
                                                                    imageData.height,
                                                                    level,
                                                                    0,
                                                                    maxResolution));
        const REAL scale(imageData.width > imageData.height
                             ? (REAL)nMaxResolution / imageData.width
                             : (REAL)nMaxResolution / imageData.height);
        ...
        FOREACH(idxVertex, mesh.vertices)
        {
            ...
            if (depthMap.isInsideWithBorder<float, 1>(x)
                && xz.z * thFrontDepth < depthMap(ROUND2INT(x)))
            {
#ifdef SCENE_USE_OPENMP
    #pragma omp critical
#endif
...
bool Scene::ExportMeshToDepthMaps(const String &baseName)
{
    ...
    FOREACH(idxImage, images)
    {
        ...
        if (!image.IsValid())
            continue;
        const unsigned imageSize(image.RecomputeMaxResolution(OPTDENSE::nResolutionLevel,
        ...
        const String fileName(Util::insertBeforeFileExt(baseName,
                                                        String::FormatString("%04u", image.ID)));
        if ((nType == 2 && ![&]() {
...
bool Scene::EstimateNeighborViewsPointCloud(unsigned maxResolution)
{
    ...
    const auto ProjectGridToImage = [&](IIndex idI, IIndex idJ, Depth depth)
    {
        ...
        for (unsigned r = 0; r < maxResolution; ++r)
        {
            for (unsigned c = 0; c < maxResolution; ++c)
            {
                ...
                pointcloud.pointViews.emplace_back(idI < idJ ? PointCloud::ViewArr{idI, idJ}
                                                             : PointCloud::ViewArr{idJ, idI});
            }
        }
    };
    pointcloud.Release();
    FOREACH(i, images)
    {
        ...
        FOREACH(j, images)
        {
            ...
            Point3 X;
            TriangulatePoint3D(imageData.camera.K,
                               imageData2.camera.K,
                               imageData.camera.R,
                               imageData2.camera.R,
                               imageData.camera.C,
...
bool Scene::SelectNeighborViews(uint32_t ID,
                                IndexArr &points,
                                unsigned nMinViews,
                                unsigned nMinPointViews,
                                float fOptimAngle,
                                unsigned nInsideROI)
{
    ASSERT(points.empty());
    ...
    FOREACH(idx, pointcloud.points)
    {
        ...
        for (const PointCloud::View &view : views)
        {
            ...
            const float wAngle(EXP(SQUARE(fAngle - fOptimAngle)
                                   * (fAngle < fOptimAngle ? sigmaAngleSmall : sigmaAngleLarge)));
    ...
    if (neighbors.empty())
    {
        ...
        FOREACH(IDB, images)
        {
            ...
            for (uint32_t idx : points)
            {
                ...
                if (!imageData.camera.IsInside(ptA, boundsA)
                    || !imageDataB.camera.IsInside(ptB, boundsB))
                    projs.RemoveLast();
            }
            ASSERT(projs.size() <= score.points);
            if (projs.empty())
                continue;
            const float area(ComputeCoveredArea<float, 2, 16, false>((const float *)projs.data(),
                                                                     projs.size(),
                                                                     boundsA.ptr()));
            projs.Empty();
            // store image score
        ...
        neighbors.Sort(
            [](const ViewScore &i, const ViewScore &j)
            {
        ...
        if (VERBOSITY_LEVEL > 2)
        {
            String msg;
            FOREACH(n, neighbors)
            msg += String::FormatString(" %3u(%upts,%.2fscl)",
                                        neighbors[n].ID,
                                        neighbors[n].points,
                                        neighbors[n].scale);
            VERBOSE("Reference image %3u sees %u views:%s (%u shared points)",
                    ID,
                    neighbors.size(),
                    msg.c_str(),
                    nPoints);
...
void Scene::SelectNeighborViews(unsigned nMinViews,
                                unsigned nMinPointViews,
                                float fOptimAngle,
                                unsigned nInsideROI)
{
...
bool Scene::FilterNeighborViews(ViewScoreArr &neighbors,
                                float fMinArea,
                                float fMinScale,
                                float fMaxScale,
                                float fMinAngle,
                                ...
                                unsigned nMaxViews)
{
    ...
    RFOREACH(n, neighbors)
    {
        ...
        if (neighbors.size() > nMinViews
            && (neighbor.area < fMinArea || !ISINSIDE(neighbor.scale, fMinScale, fMaxScale)
...
{
    static const char mlp_header[]     = "<!DOCTYPE MeshLabDocument>\n"
                                         "<MeshLabProject>\n"
                                         " <MeshGroup>\n"
                                         ...
                                         "  </MLMesh>\n"
                                         " </MeshGroup>\n";
    static const char mlp_raster_pos[] = "  <MLRaster label=\"%s\">\n"
                                         "   <VCGCamera TranslationVector=\"%0.6g %0.6g %0.6g 1\"";
    static const char mlp_raster_cam[] = " LensDistortion=\"%0.6g %0.6g\""
                                         " ViewportPx=\"%u %u\""
                                         " PixelSizeMm=\"1 %0.4f\""
                                         " FocalMm=\"%0.4f\""
                                         " CenterPx=\"%0.4f %0.4f\"";
    static const char mlp_raster_rot[] = " RotationMatrix=\"%0.6g %0.6g %0.6g 0 %0.6g %0.6g %0.6g "
                                         "0 %0.6g %0.6g %0.6g 0 0 0 0 1\"/>\n"
    ...
    f.print(mlp_header,
            Util::getFileName(fileNameScene).c_str(),
            MAKE_PATH_REL(WORKING_FOLDER_FULL, fileNameScene).c_str());
...
bool Scene::ExportLinesPLY(const String &fileName,
                           const CLISTDEF0IDX(Line3f, uint32_t) & lines,
                           const Pixel8U *colors,
                           bool bBinary)
{
    ...
    static const char *elem_names[] = {"vertex", "edge"};

    // create PLY object
    ASSERT(!fileName.empty());
    Util::ensureFolder(fileName);
    const size_t memBufferSize(2 * (8 * 3 /*pos*/ + 3 * 3 /*color*/ + 6 /*space*/ + 2 /*eol*/)
                               + 2048 /*extra size*/);
    PLY ply;
...
unsigned Scene::Split(ImagesChunkArr &chunks, float maxArea, int depthMapStep) const
{
    ...
    Unsigned32Arr imageAreas(images.size());
    {
        ...
        FOREACH(idxImage, images)
        {
            ...
            for (int r = (depthData.depthMap.rows % depthMapStep) / 2; r < depthData.depthMap.rows;
                 r += depthMapStep)
            {
                for (int c = (depthData.depthMap.cols % depthMapStep) / 2;
                     c < depthData.depthMap.cols;
                     c += depthMapStep)
                {
    ...
    FOREACH(c, chunks)
    {
        ...
        {
            const float areaRatio(static_cast<float>(chunkImageAreas[idxImage])
                                  / static_cast<float>(imageAreas[idxImage]));
        ...
        for (auto it = chunk.images.begin(); it != chunk.images.end();)
        {
            ...
            if (static_cast<float>(chunkImageAreas[idxImage])
                    / static_cast<float>(imageAreas[idxImage])
                < minImageContributionRatioChunk)
        ...
        FOREACH(cLarge, chunks)
        {
            ...
            for (auto it = chunkSmall.images.begin(); it != chunkSmall.images.end();)
            {
                ...
                if (chunkSmallImageAreas[idxImage] < chunkLargeImageAreas[idxImage]
                    && static_cast<float>(chunkLargeImageAreas[idxImage])
                               / static_cast<float>(imageAreas[idxImage])
                           > minImageContributionRatioLargerChunk)
        ...
        FOREACH(cLarge, chunks)
        {
            ...
            if (numLargestCommonImages < numCommonImages
                || (numLargestCommonImages == numCommonImages
                    && chunks[idxBestChunk].images.size() < chunkLarge.images.size()))
            {
                numLargestCommonImages = numCommonImages;
                idxBestChunk           = cLarge;
            }
    ...
    DEBUG_EXTRA("Scene split (%g max-area): %u chunks (%s)",
                maxArea,
                chunks.size(),
                TD_TIMER_GET_FMT().c_str());
#if 0 || defined(_DEBUG)
...
bool Scene::ExportChunks(const ImagesChunkArr &chunks, const String &path, ARCHIVE_TYPE type) const
{
    FOREACH(chunkID, chunks)
    {
        ...
        // serialize out the current state
        if (!subset.Save(String::FormatString("%s" PATH_SEPARATOR_STR "scene_%04u.mvs",
                                              path.c_str(),
                                              chunkID),
                         type))
            return false;
    ...
    FOREACH(idx, images)
    {
        ...
        if (!folderName.empty())
        {
            ...
            image.name = folderName
                         + String::FormatString("%05u%s",
                                                image.ID,
                                                Util::getFileExt(image.name).c_str());
...
void Scene::Transform(const Matrix3x4 &transform)
{
    ...
    RQDecomp3x3<REAL>(cv::Mat(3,
                              4,
                              cv::DataType<REAL>::type,
                              const_cast<REAL *>(transform.val))(cv::Rect(0, 0, 3, 3)),
                      mscale,
                      rotation);
...
#endif
    ASSERT(mscale(0, 0) > 0 && ISEQUAL(mscale(0, 0), mscale(1, 1))
           && ISEQUAL(mscale(0, 0), mscale(2, 2)));
...
// estimate ground plane, transform scene such that it is positioned at origin, and compute the
// volume of the mesh;
...
REAL Scene::ComputeLeveledVolume(float planeThreshold, float sampleMesh, unsigned upAxis, bool verbose)
{
    ...
    if (planeThreshold >= 0 && !mesh.IsWatertight())
    {
        ...
        TD_TIMER_START();
        Planef groundPlane(mesh.EstimateGroundPlane(images,
                                                    sampleMesh,
                                                    planeThreshold,
                                                    verbose ? MAKE_PATH("ground_plane.ply")
                                                            : String()));
        ...
        const Point3 translation(rotation
                                 * -Cast<REAL>(Point3f(groundPlane.ProjectPoint(mesh.GetCenter()))));
        const REAL scale(1);
        Transform(rotation, translation, scale);
    }
...
bool Scene::EstimateROI(int nEstimateROI, float scale)
{
    ...
    {
        VERBOSE("The camera directions mean is unbalanced; the scene will be considered unbounded "
                "(no ROI)");
        return false;
    }
    DEBUG_ULTIMATE("The camera positions median is (%f,%f,%f), directions mean and norm are "
                   "(%f,%f,%f), %f",
                   camCenter.x,
                   camCenter.y,
                   camCenter.z,
    ...
    FOREACH(i, cameras)
    {
        ...
        {
            VERBOSE("Found a camera not pointing towards the scene center; the scene will be "
                    "considered unbounded (no ROI)");
            return false;
        }
...
bool Scene::ComputeTowerCylinder(Point2f &centerPoint,
                                 float &fRadius,
                                 float &fROIRadius,
                                 float &zMin,
                                 float &zMax,
                                 ...
                                 const int towerMode)
{
    ...
    if (quality.y / quality.z > 0.6f || quality.x / quality.y < 0.8f)
    {
        ...
        if (towerMode > 0)
        {
            DEBUG_ULTIMATE("error: does not seem to be a tower: X(%.2f), Y(%.2f), Z(%.2f)",
                           quality.x,
                           quality.y,
                           quality.z);
            return false;
...
size_t Scene::DrawCircle(PointCloud &pc,
                         PointCloud::PointArr &outCircle,
                         const Point3f &circleCenter,
                         const float circleRadius,
                         const unsigned nTargetPoints,
                         const float fStartAngle,
                         const float fAngleBetweenPoints)
{
    outCircle.Release();
    for (unsigned pIdx = 0; pIdx < nTargetPoints; ++pIdx)
    {
        ...
        FOREACH(idxImg, images)
        {
            ...
            const Point2f x(xz.x, xz.y);
            if (!Image8U::isInside<float>(x, image.GetSize()) || xz.z <= 0)
                continue;
...
PointCloud Scene::BuildTowerMesh(const PointCloud &origPointCloud,
                                 const Point2f &centerPoint,
                                 const float fRadius,
                                 const float fROIRadius,
                                 const float zMin,
                                 const float zMax,
                                 const float minCamZ,
                                 bool bFixRadius)
{
    ...
    const unsigned nTargetCircles(ROUND2INT((zMax - zMin)
                                            * nTargetDensity)); // how many circles in cylinder
    const float fCircleFrequence((zMax - zMin)
                                 / nTargetCircles); // the distance between neighbor circles
    PointCloud towerPC;
    PointCloud::PointArr circlePoints;
    Mesh::VertexVerticesArr meshCircles;
    if (bFixRadius)
    {
        const unsigned nTargetPoints(MAX(10,
                                         ROUND2INT(FTWO_PI * fRadius
                                                   * nTargetDensity))); // how many points on each circle
        const float fAngleBetweenPoints(FTWO_PI
                                        / nTargetPoints); // the angle between neighbor points on the circle
        for (unsigned cIdx = 0; cIdx < nTargetCircles; ++cIdx)
        {
            const Point3f circleCenter(centerPoint,
                                       zMin + fCircleFrequence * cIdx); // center point of the circle
            const float fStartAngle(fAngleBetweenPoints
                                    * SEACAVE::random()); // starting angle for the first point
            DrawCircle(towerPC,
                       circlePoints,
                       circleCenter,
                       fRadius,
    ...
    else
    {
        ...
        for (unsigned cIdx = 0; cIdx < nTargetCircles; ++cIdx)
        {
            ...
            else
            {
                ...
                {
                    pDistances.Sort();
                    const size_t topIdx(MIN(pDistances.size() - 1,
                                            CEIL2INT<size_t>(pDistances.size() * 0.95f)));
        ...
        if (circleRadii.size() > 2)
        {
            for (size_t ri = 1; ri < circleRadii.size() - 1; ++ri)
            {
                ...
                if (ri > 2 && ri < circleRadii.size() - 5)
                {
                    ...
                    circleRadius = ABS(medianRadius - aboveRad) < ABS(medianRadius - belowRad)
                                       ? aboveRad
                                       : belowRad;
                }
                else
        ...
        FOREACH(rIdx, circleRadii)
        {
            ...
            const unsigned nTargetPoints(MAX(10,
                                             ROUND2INT(FTWO_PI * circleRadius
                                                       * nTargetDensity))); // how many points on each circle
            const float fAngleBetweenPoints(FTWO_PI
                                            / nTargetPoints); // the angle between neighbor points on the circle
            const float fStartAngle(fAngleBetweenPoints
...
//  - towerMode:  0 - disabled, 1 - replace, 2 - append, 3 - select neighbors, 4 - select neighbors
//  and append, <0 - force tower mode
void Scene::InitTowerScene(const int towerMode)
{
    ...
    PointCloud
        towerPC(BuildTowerMesh(pointcloud, centerPoint, fRadius, fROIRadius, zMin, zMax, minCamZ, false));
    mesh.Release();
    ...
    case 1: // replace
        ...
        VERBOSE("Scene identified as tower-like; replace existing point-cloud with detected tower "
                "point-cloud");
        break;
    case 2: // append
        AppendPointCloud(towerPC);
        VERBOSE("Scene identified as tower-like; append to existing point-cloud the detected tower "
                "point-cloud");
        break;
    case 3: // select neighbors
        pointcloud.Swap(towerPC);
        SelectNeighborViews(OPTDENSE::nMinViews,
                            OPTDENSE::nMinViewsTrustPoint > 1 ? OPTDENSE::nMinViewsTrustPoint : 2,
                            FD2R(OPTDENSE::fOptimAngle),
                            OPTDENSE::nPointInsideROI);
        pointcloud.Swap(towerPC);
        VERBOSE("Scene identified as tower-like; only select view neighbors from detected tower "
                "point-cloud");
        break;
    case 4: // select neighbors and append tower points
        ...
        pointcloud.Swap(towerPC);
        AppendPointCloud(towerPC);
        VERBOSE("Scene identified as tower-like; select view neighbors from detected tower "
                "point-cloud and next append it to existing point-cloud");
        break;
    }
} // InitTowerScene
