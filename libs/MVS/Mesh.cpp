...
// vertexFaces每个顶点对应的面的集合
// 遍历所有的面，将序列放入面所对应的三角顶点的数组中
...
void Mesh::ListIncidenteFaceFaces()
{
    ...
    struct face_back_inserter_t
    {
        ...
        inline face_back_inserter_t &operator*()
        {
            return *this;
        }
    ...
    FOREACH(f, faces)
    {
        ...
        const FaceIdxArr *const pFaces[] = {
            &vertexFaces[face[0]], &vertexFaces[face[1]], &vertexFaces[face[2]]};
        ...
        for (int v = 0; v < 3; ++v)
        {
            ...
            std::set_intersection(facesI.begin(),
                                  facesI.end(),
                                  facesJ.begin(),
                                  facesJ.end(),
                                  faceBackInserter);
...
void Mesh::ComputeNormalFaces()
{
    ...
    if (kernelComputeFaceNormal.IsValid())
    {
        reportCudaError(kernelComputeFaceNormal((int)faces.size(),
                                                ...
                                                faces.size()));
        reportCudaError(kernelComputeFaceNormal.GetResult(0, faceNormals));
...
void Mesh::SmoothNormalFaces(float fMaxGradient, float fOriginalWeight, unsigned nIterations)
{
    ...
    for (unsigned rep = 0; rep < nIterations; ++rep)
    {
        ...
        FOREACH(idxFace, faces)
        {
            ...
            const Normal newFaceNormal      = normalized(originalNormal * fOriginalWeight
                                                    + avgNeighborsNormal * (1.f - fOriginalWeight));
...
struct UsedTypes
    : public vcg::UsedTypes<vcg::Use<Vertex>::AsVertexType, vcg::Use<Edge>::AsEdgeType, vcg::Use<Face>::AsFaceType>
{
};
...
public:
    ...
    static vcg::math::Quadric<double> &Qd(Vertex &v)
    {
        return TD()[v];
    }
    ...
    static Vertex::ScalarType W(Vertex * /*v*/)
    {
        return 1.0;
    }
...
class TriEdgeCollapse
    : public vcg::tri::TriEdgeCollapseQuadric<Mesh, VertexPair, TriEdgeCollapse, QHelper>
{
public:
...
void Mesh::Clean(float fDecimate,
                 float fSpurious,
                 bool bRemoveSpikes,
                 ...
                 bool bLastClean)
{
    ...
    CLEAN::Mesh mesh;
    {
        CLEAN::Mesh::VertexIterator vi = vcg::tri::Allocator<CLEAN::Mesh>::AddVertices(mesh,
                                                                                       vertices.size());
    ...
    if (fDecimate < 1)
    {
        ...
        pp.QualityThr = 0.3; // Quality Threshold for penalizing bad shaped faces: the value is in
                             // the range [0..1], 0 accept any kind of face (no penalties), 0.5
                             // penalize faces with quality < 0.5, proportionally to their shape
        pp.PreserveBoundary = false; // the simplification process tries to not affect mesh
                                     // boundaries during simplification
        pp.PreserveTopology = false; // avoid all collapses that cause a topology change in the mesh
                                     // (like closing holes, squeezing handles, etc); if checked the
                                     // genus of the mesh should stay unchanged
        pp.QualityWeight = false; // use the Per-Vertex quality as a weighting factor for the simplification:
                                  // the weight is used as an error amplification value, so a vertex
                                  // with a high quality value will not be simplified and a portion
                                  // of the mesh with low quality values will be aggressively simplified
        pp.NormalCheck = false;     // try to avoid face flipping effects and try to preserve the
                                    // original orientation of the surface
        pp.OptimalPlacement = true; // each collapsed vertex is placed in the position minimizing
                                    // the quadric error; it can fail (creating bad spikes) in case
                                    // of very flat areas; if disabled edges are collapsed onto one
                                    // of the two original vertices and the final mesh is composed
                                    // by a subset of the original vertices
        pp.QualityQuadric = false; // add additional simplification constraints that improves the quality
                                   // of the simplification of the planar portion of the mesh
        // decimate
    ...
    // clean mesh
    {
        ...
        for (int i = 0; i < 10; ++i)
        {
            vcg::tri::UpdateTopology<CLEAN::Mesh>::FaceFace(mesh);
            vcg::tri::UpdateTopology<CLEAN::Mesh>::VertexFace(mesh);
            const int nSplitNonManifoldVertices = vcg::tri::Clean<
                CLEAN::Mesh>::SplitNonManifoldVertex(mesh, 0.1f);
    ...
    if (fSpurious > 0)
    {
        ...
        const size_t numLongFaces(vcg::tri::UpdateSelection<CLEAN::Mesh>::FaceOutOfRangeEdge(mesh,
                                                                                             0,
                                                                                             thLongEdge));
        ...
        const std::pair<int, int>
            delInfo(vcg::tri::Clean<CLEAN::Mesh>::RemoveSmallConnectedComponentsDiameter(mesh,
                                                                                         thLongEdge));
    ...
    if (bRemoveSpikes)
    {
        ...
        while (true)
        {
            ...
            for (CLEAN::Mesh::FaceIterator fi = mesh.face.begin(); fi != mesh.face.end(); ++fi)
            {
                if (!fi->IsD() && (fi->V(0)->IsD() || fi->V(1)->IsD() || fi->V(2)->IsD()))
                    vcg::tri::Allocator<CLEAN::Mesh>::DeleteFace(mesh, *fi);
            }
    ...
    if (nCloseHoles > 0)
    {
        ...
        const int holeCnt(vcg::tri::Hole<CLEAN::Mesh>::EarCuttingIntersectionFill<
                          vcg::tri::SelfIntersectionEar<CLEAN::Mesh>>(mesh, (int)nCloseHoles, false));
#else
        const int holeCnt = vcg::tri::Hole<CLEAN::Mesh>::EarCuttingFill<
            vcg::tri::MinimumWeightEar<CLEAN::Mesh>>(mesh, (int)nCloseHoles, false);
#endif
...
// account for diferences in pixel center convention: while OpenMVS uses the same convention as
// OpenCV and DirectX 9 where the center of a pixel is defined at integer coordinates, i.e. the
// center is at (0, 0) and the top left corner is at (-0.5, -0.5), DirectX 10+, OpenGL, and Vulkan
// convention is the center of a pixel is defined at half coordinates, i.e. the center is at (0.5,
// 0.5) and the top left corner is at (0, 0)
...
void Mesh::FaceTexcoordsNormalize(TexCoordArr &newFaceTexcoords, bool flipY) const
{
    ...
    else
    {
        ...
        FOREACH(i, texturesDiffuse)
        {
            ...
            invNorms[i] = TexCoord(1.f / (float)texturesDiffuse[i].cols,
                                   1.f / (float)texturesDiffuse[i].rows);
        }
        if (flipY)
        {
            FOREACH(i, faceTexcoords)
            {
                ...
                newFaceTexcoords[i]      = TexCoord((texcoord.x + halfPixel.x) * invNorm.x,
    ...
    else
    {
        ...
        if (flipY)
        {
            FOREACH(i, faceTexcoords)
            {
                ...
                newFaceTexcoords[i]      = TexCoord(texcoord.x * scale.x - halfPixel.x,
...
struct Vertex
{
    ...
    static void InitLoadProps(PLY &ply,
                              int elem_count,
                              Mesh::VertexArr &vertices,
                              Mesh::NormalArr &vertexNormals)
    {
        ...
        for (size_t p = 0; p < nMaxProps; ++p)
        {
            ...
            {
            case 0:
                vertices.resize((IDX)elem_count);
                break;
            case 3:
                vertexNormals.resize((IDX)elem_count);
                break;
            }
        }
    }
    ...
    static void InitSaveProps(PLY &ply, int elem_count, bool bNormals)
    {
...
struct Face
{
    ...
    static void InitLoadProps(PLY &ply,
                              int elem_count,
                              Mesh::FaceArr &faces,
                              Mesh::TexCoordArr &faceTexcoords,
                              Mesh::TexIndexArr &faceTexindices)
    {
        ...
        for (size_t p = 0; p < nMaxProps; ++p)
        {
            ...
            {
            case 0:
                faces.resize((IDX)elem_count);
                break;
            case 1:
                faceTexcoords.resize((IDX)elem_count * 3);
                break;
            case 2:
                faceTexindices.resize((IDX)elem_count);
                break;
            }
        }
    }
    ...
    static void InitSaveProps(PLY &ply,
                              int elem_count,
                              bool bFaces,
                              bool bTexcoord,
                              bool bTexnumber,
                              bool bFaceweight = false)
    {
        if (bFaces)
...
const PLY::PlyProperty Face::props[] = {
    {"vertex_indices",
     PLY::Uint32,
     PLY::Uint32,
     offsetof(Face, face.pFace),
     1,
     PLY::Uint8,
     PLY::Uint8,
     offsetof(Face, face.num)},
    {"texcoord",
     PLY::Float32,
     PLY::Float32,
     offsetof(Face, tex.pTex),
     1,
     PLY::Uint8,
     PLY::Uint8,
     offsetof(Face, tex.num)},
...
bool Mesh::Load(const String &fileName)
{
    ...
    if (!ret)
        return false;
    DEBUG_EXTRA("Mesh loaded: %u vertices, %u faces (%s)",
                vertices.size(),
                faces.size(),
                TD_TIMER_GET_FMT().c_str());
    return true;
        ...
        else if (PLY::equal_strings(BasicPLY::elem_names[1], elem_name))
        {
            ...
            FOREACH(f, faces)
            {
                ...
                /*if (!faceTexcoords.empty())
                {
                    if (face.tex.num != 6)
                    {
                        DEBUG_EXTRA("error: unsupported mesh file (texture coordinates not per "
                                    "face vertex)");
                        return false;
                    }
            ...
            //             texturesDiffuse.emplace_back().Load(Util::getFilePath(fileName)
            //                                                 + textureFileName);
            //         }
            //     }
...
bool Mesh::LoadGLTF(const String &fileName, bool bBinary)
{
    ...
    tinygltf::Model gltfModel;
    {
        ...
        std::string err, warn;
        if (bBinary ? !loader.LoadBinaryFromFile(&gltfModel, &err, &warn, fileName)
        ...
        for (const tinygltf::Primitive &gltfPrimitive : gltfMesh.primitives)
        {
            ...
            // read vertices
            {
                const tinygltf::Accessor
                    &gltfAccessor = gltfModel.accessors[gltfPrimitive.attributes.at("POSITION")];
                ...
                const uint8_t *pData           = buffer.data.data() + gltfBufferView.byteOffset
                                       + gltfAccessor.byteOffset;
...
bool Mesh::Save(const String &fileName, const cList<String> &comments, bool bBinary) const
{
    ...
    if (!ret)
        return false;
    DEBUG_EXTRA("Mesh saved: %u vertices, %u faces (%s)",
                vertices.size(),
                faces.size(),
                TD_TIMER_GET_FMT().c_str());
    return true;
...
bool Mesh::SavePLY(const String &fileName,
                   const cList<String> &comments,
                   bool bBinary,
                   bool bTexLossless) const
{
    ASSERT(!fileName.empty());
    ...
    if (!ply.write(fileName, 2, BasicPLY::elem_names, PLY::ASCII))
    {
    ...
    if (HasTexture())
    {
        ...
        {
            const String textureFileName(Util::getFileFullName(fileName)
                                         + std::to_string(texId).c_str()
                                         + (bTexLossless ? _T(".png") : _T(".jpg")));
    ...
    BasicPLY::Face::InitSaveProps(ply,
                                  (int)faces.size(),
                                  !faces.empty(),
                                  !faceTexcoords.empty(),
                                  !faceTexindices.empty());
...
bool Mesh::SaveOBJ(const String &fileName) const
{
    ...
    if (!vertexNormals.empty())
    {
        ...
        model.get_normals().insert(model.get_normals().begin(),
                                   vertexNormals.begin(),
                                   vertexNormals.end());
    }
    ...
    if (!faceTexcoords.empty())
    {
        ...
        model.get_texcoords().insert(model.get_texcoords().begin(),
                                     normFaceTexcoords.begin(),
                                     normFaceTexcoords.end());
    }
...
template <typename T>
void ExtendBufferGLTF(const T *src,
                      size_t size,
                      tinygltf::Buffer &dst,
                      size_t &byte_offset,
                      size_t &byte_length)
...
bool Mesh::SaveGLTF(const String &fileName, bool bBinary) const
{
    ...
    for (size_t meshId = 0; meshId < meshes.size(); meshId++)
    {
        ...
        // setup vertices
        {
            STATIC_ASSERT(3 * sizeof(Vertex::Type)
                          == sizeof(Vertex)); // VertexArr should be continuous
            ...
            ExtendBufferGLTF(mesh.vertices.data(),
                             mesh.vertices.size(),
                             gltfBuffer,
        ...
        // setup faces
        {
            ...
            ExtendBufferGLTF(mesh.faces.data(),
                             mesh.faces.size(),
                             gltfBuffer,
                             triangleBufferView.byteOffset,
                             triangleBufferView.byteLength);
        ...
        if (mesh.HasTexture())
        {
            ...
            STATIC_ASSERT(2 * sizeof(TexCoord::Type)
                          == sizeof(TexCoord)); // TexCoordArr should be continuous
            ...
            ExtendBufferGLTF(normFaceTexcoords.data(),
                             normFaceTexcoords.size(),
                             gltfBuffer,
            ...
            mesh.texturesDiffuse[0].copyTo(cv::Mat(mesh.texturesDiffuse[0].size(),
                                                   CV_8UC3,
                                                   image.image.data()));
    ...
    struct Tools
    {
        static bool WriteImageData(const std::string *basepath,
                                   ...
                                   void *)
        {
            ...
            image->uri = Util::isFullPath(filename->c_str())
                             ? Util::getRelativePath(*basepath, *filename)
                             : String(*filename);
            String basePath(*basepath);
            return cv::imwrite(Util::ensureFolderSlash(basePath) + image->uri,
    ...
    return gltf.WriteGltfSceneToFile(&gltfModel,
                                     fileName,
                                     bEmbedImages,
                                     bEmbedBuffers,
                                     bPrettyPrint,
...
bool Mesh::Save(const FacesChunkArr &chunks,
                ...
                bool bBinary) const
{
    ...
    FOREACH(i, chunks)
    {
        ...
        if (!mesh.Save(Util::insertBeforeFileExt(fileName, String::FormatString("_chunk%02u", i)),
                       comments,
                       bBinary))
            return false;
    }
...
public:
    ...
    void setBorder()
    {
        flags.set(FLG_BORDER);
    }

    void unsetBorder()
    {
        flags.unset(FLG_BORDER);
    }

    bool isBorder() const
    {
        return flags.isSet(FLG_BORDER);
    }
...
public:
    ...
    char removal_status; // for self intersection removal: U -  unvisited; 'P' - partially valid; V
                         // - valid for connected components: U - unvisited; V - visited
...
inline double OppositeAngle(Vertex::Halfedge_handle h)
{
    ...
    return v_angle(h->vertex()->point() - h->next()->vertex()->point(),
                   h->prev()->vertex()->point() - h->next()->vertex()->point());
}
...
inline bool CanCollapseCenterVertex(Vertex::Vertex_handle v)
{
    ...
    if (v->isBorder())
        return false;
    return (v->halfedge()->prev()->opposite()->facet()
            != v->halfedge()->opposite()->next()->opposite()->facet());
}
...
static bool CanCollapseEdge(Vertex::Halfedge_handle v0v1)
{
    ...
    c = v0->vertex_begin();
    d = c;
    CGAL_For_all(c, d) c->opposite()->vertex()->flags.unset(Vertex::FLG_EULER);

    c = v1->vertex_begin();
    d = c;
    CGAL_For_all(c, d) c->opposite()->vertex()->flags.set(Vertex::FLG_EULER);
...
static bool CanFlipEdge(Vertex::Halfedge_handle h)
{
    ...
    if ((h->next() == null_h) || (h->prev() == null_h) || (h->opposite() == null_h)
        || (h->opposite()->next() == null_h))
        return false;
    ...
    Vector v_perp_1 = CGAL::cross_product(h->vertex()->point() - h->next()->vertex()->point(),
                                          h->vertex()->point() - h->prev()->vertex()->point());
    // Vector v_perp_2 =
    // CGAL::cross_product(h->opposite()->vertex()->point()-h->opposite()->next()->vertex()->point(),h->opposite()->vertex()->point()-h->opposite()->prev()->vertex()->point());
    Vector v_perp_2 = CGAL::cross_product(h->opposite()->next()->vertex()->point()
    ...
    case 3:
    {
        ...
        do
        {
            ...
            for (Edge_iterator ei = p.edges_begin(); ei != p.edges_end(); ++ei)
            {
                ...
                float cost1((float)MINF(MINF(MINF(MINF(MINF(p_angle(p3, p1, p2), p_angle(p1, p3, p2)),
                                                       p_angle(p4, p1, p3)),
                                                  p_angle(p4, p3, p1)),
                                             p_angle(p1, p4, p2)),
                                        p_angle(p1, p2, p3)));
                float cost2((float)MINF(MINF(MINF(MINF(MINF(p_angle(p1, p2, p4), p_angle(p1, p4, p2)),
                                                       p_angle(p3, p4, p2)),
                                                  p_angle(p4, p2, p3)),
                                             p_angle(p4, p1, p2)),
                                        p_angle(p4, p3, p2)));
...
//  - collapseRatio
//     - for small angles: given the corresponding edges (a,b,c) in all permutations, if (a/b <
//     collapseRatio) & (a/c < collapseRatio)
//     - a good value to use is 0.1
static int FixDegeneracy(Polyhedron &p, double collapseRatio, double degenerateAngleDeg)
{
    DEBUG_LEVEL(3,
                "Fix degeneracy: %g collapse-ratio, %g degenerate-angle",
                collapseRatio,
                degenerateAngleDeg);
    double edge[3];
    ...
    for (Facet_iterator fi = p.facets_begin(); fi != p.facets_end();)
    {
        ...
        if (fi->triangle().is_degenerate())
        {
            ...
            DEBUG_LEVEL(3,
                        "Degenerate angle: %g (avg edge %g)",
                        v_angle(fi->get_point(0) - fi->get_point(1),
                                fi->get_point(0) - fi->get_point(2)),
                        avg_edge);
            const double delta(avg_edge * 0.01);
            fi->halfedge()->vertex()->point() = fi->halfedge()->vertex()->point()
                                                + Vector(0, 0, delta);
            fi->halfedge()->next()->vertex()->point() = fi->halfedge()->next()->vertex()->point()
                                                        + Vector(0, delta, 0);
            fi->halfedge()
                ->next()
                ->next()
                ->vertex()
                ->point() = fi->halfedge()->next()->next()->vertex()->point() + Vector(delta, 0, 0);
        }
        ...
        for (i = 0; i < 3; i++)
        {
            if ((edge[i] / edge[(i + 1) % 3] < collapseRatio)
                && (edge[i] / edge[(i + 2) % 3] < collapseRatio))
            {
...
static void ComputeConnectedComponents(Polyhedron &p,
        ...
        { // start a new component
            ...
            { // fill the current component
                ...
                do
                {
                    ...
                    float edge_size       = (float)v_norm(h->vertex()->point()
                                                    - h->prev()->vertex()->point());
...
static void RemoveConnectedComponents(Polyhedron &p, int size_threshold, float edge_threshold)
{
    ...
    for (std::vector<MeshConnectedComponent>::iterator vi = connected_components.begin();
         vi != connected_components.end();)
    {
        if ((vi->size <= size_threshold) || (vi->edge_max <= edge_threshold)
            || ((vi->area < edge_threshold * edge_threshold * PI * 2) && (vi->is_open == false)))
        {
...
// Description:
// - The goal of this method is to ensure that all the edges of the mesh are within the interval
// [epsilonMin,epsilonMax].
//   In order to do so, edge collapses and edge split operations are performed.
// - The method also attempts to fix degeneracies by invoking
// FixDegeneracy(collapseRatio,degenerate_angle_deg) and performs some local smoothing, based on the
// operating mode. Parameters:
// - [epsilonMin, epsilonMax] - the desired edge interval (negative if to be used as multiplier of
// the initial mean edge length)
// - collapseRatio, degenerate_angle_deg - parameters used to invoke FixDegeneracy (see function for
// more details)
// - mode : 0 - fixDegeneracy=No  smoothing=Yes;
//          1 - fixDegeneracy=Yes smoothing=Yes; (default)
//         10 - fixDegeneracy=Yes smoothing=No;
// - max_iter (default=30) - maximum number of iterations to be performed; since there is no
// guarantee that one operations (such as a collapse, for example)
//   will not in turn generate new degeneracies, operations are being performed on the mesh in an
//   iterative fashion.
static void EnsureEdgeSize(Polyhedron &p,
                           double epsilonMin,
                           double epsilonMax,
                           double collapseRatio,
                           double degenerate_angle_deg,
                           int mode,
                           int max_iters,
                           int comp_size_threshold)
{
    if (mode > 0)
    ...
    DEBUG_LEVEL(3,
                "Ensuring edge size in [%g, %g] with edges currently in [%g, %g]",
                epsilonMin,
                epsilonMax,
                edge.min,
                edge.max);
    ...
    while ((edge.min < epsilonMin || edge.max > epsilonMax) && (no_ops > 0) && (++iters < max_iters))
    {
        ...
        for (EdgeScoreArr::IDX i = 0; i < bigEdges.size(); ++i)
        {
            ...
            if (!CanSplitEdge(h))
                continue;
#if CURVATURE_TH > 0
            const float avg_mean_curv(MAXF(ABS(h->vertex()->mean_curvature),
                                           ABS(h->prev()->vertex()->mean_curvature)));
    ...
    {
        ComputeStatsEdge(p, edge);
        VERBOSE("Edge size in [%g, %g] (requested in [%g, %g]): %d ops, %d iters",
                edge.min,
                edge.max,
                epsilonMin,
                epsilonMax,
...
#if ROBUST_NORMALS > 0
static std::vector<std::pair<Vertex *, int>> GetRingNeighbourhood(Vertex &v,
                                                                  int ring_size,
...
static void ComputeVertexRobustNormal(Vertex &v, int maxRings)
{
    ...
    for (std::vector<std::pair<Vertex *, int>>::const_iterator n_it = neighs.cbegin();
         n_it != neighs.cend();
         ++n_it)
    {
...
template <class HDS, class K> class TMeshBuilder : public CGAL::Modifier_base<HDS>
{
public:
    ...
    TMeshBuilder(const Mesh::VertexArr &_vertices, const Mesh::FaceArr &_faces)
        : vertices(_vertices), faces(_faces), bProblems(false)
    {
    }
...
#if TD_VERBOSE != TD_VERBOSE_OFF
        if (bProblems)
            DEBUG_EXTRA("warning: ignoring the following facet(s) violating the manifold "
                        "constraint:%s",
                        msgFaces.c_str());
#endif
...
static bool ImportMesh(Polyhedron &p, const Mesh::VertexArr &vertices, const Mesh::FaceArr &faces)
{
    ...
    p.delegate(builder);
    UpdateMeshData(p);
    DEBUG_ULTIMATE("Mesh imported: %u vertices, %u facets (%u border edges)",
                   p.size_of_vertices(),
                   p.size_of_facets(),
...
static bool ExportMesh(const Polyhedron &p, Mesh::VertexArr &vertices, Mesh::FaceArr &faces)
{
    ...
    for (Polyhedron::Vertex_const_iterator it = p.vertices_begin(), ite = p.vertices_end(); it != ite;
         ++it)
    {
    ...
    CGAL::Inverse_index<Polyhedron::Vertex_const_iterator> index(p.vertices_begin(),
                                                                 p.vertices_end());
    ...
    DEBUG_ULTIMATE("Mesh exported: %u vertices, %u facets (%u border edges)",
                   p.size_of_vertices(),
                   p.size_of_facets(),
                   p.size_of_border_edges());
    return true;
...
void Mesh::EnsureEdgeSize(float epsilonMin,
                          float epsilonMax,
                          float collapseRatio,
                          float degenerate_angle_deg,
                          int mode,
                          int max_iters)
{
    CLN::Polyhedron p;
...
void Mesh::Subdivide(const AreaArr &maxAreas, uint32_t maxArea)
{
    ...
    struct SplitFace
    {
        ...
        inline SplitFace() : bSplit(false)
        {
            memset(idxVert, 0xFF, sizeof(VIndex) * 3);
        }
    ...
    FOREACH(f, maxAreas)
    {
        ...
        for (const auto &fc : mapFaces)
        {
            ...
            if (fc.first < f && maxAreas[fc.first] > maxAreaTh)
            {
                ...
                ASSERT(mapSplits[fc.first].idxVert[SplitFace::FindSharedEdge(faces[fc.first], face)]
                       == newface[SplitFace::FindSharedEdge(face, faces[fc.first])]);
                continue;
            }
            const VIndex idxVertex(newface[SplitFace::FindSharedEdge(face, faces[fc.first])]);
            VIndex &idxSplit = mapSplits[fc.first]
                                   .idxVert[SplitFace::FindSharedEdge(faces[fc.first], face)];
...
void Mesh::CloseHoleQuality(VertexIdxArr &verts)
{
    struct CandidateFace
    {
        ...
        CandidateFace(VIndex v0, VIndex v1, VIndex v2, const Mesh &mesh) : face(v0, v1, v2)
        {
            ...
            angle = ACOS(ComputeAngle(mesh.vertices[face[1]].ptr(),
                                      mesh.vertices[face[0]].ptr(),
                                      mesh.vertices[face[2]].ptr()));
            ...
            else
            {
                ...
                else
                {
                    ...
                    dihedral = MAXF(ACOS(ComputeAngle(n.ptr(), n0.ptr())),
                                    ACOS(ComputeAngle(n.ptr(), n1.ptr())));
                }
            }
            aspectRatio = ComputeTriangleQuality(mesh.vertices[face[0]],
                                                 mesh.vertices[face[1]],
                                                 mesh.vertices[face[2]]);
        }

        inline operator const Face &() const
        {
            return face;
        }
        ...
        {
            return aspectRatio - 0.3f /*diedral weight*/ * (dihedral / (float)M_PI);
        }

        // In the heap, by default, we retrieve the LARGEST value,
        // so if we need the ear with minimal dihedral angle, we must reverse the sign of the
        // comparison. The concave elements must be all in the end of the heap, sorted accordingly,
    ...
    {
        if (candidateFaces
                .emplace_back(verts[v], verts[(v + 1) % verts.size()], verts[(v + 2) % verts.size()], *this)
                .aspectRatio
            < 0)
...
//  - sampleMesh: uniformly samples points on the mesh (0 - disabled, <0 - number of points, >0 -
//  sample density per square unit)
//  - planeThreshold: threshold used to estimate the ground plane (0 - auto)
Planef Mesh::EstimateGroundPlane(const ImageArr &images,
                                 float sampleMesh,
                                 float planeThreshold,
                                 const String &fileExportPlane) const
{
    ASSERT(!IsEmpty());
...
void Mesh::SamplePoints(REAL samplingDensity, unsigned mumPointsTheoretic, PointCloud &pointcloud) const
{
    ...
    FOREACH(idxFace, faces)
    {
        ...
        for (unsigned i = 0; i < pointsToAdd; ++i)
        {
            ...
            // compute position
            pointcloud.points.emplace_back(O + static_cast<Vertex::Type>(x) * u
                                           + static_cast<Vertex::Type>(y) * v);
            ...
            if (HasTexture())
            {
                ...
                const TexCoord xt(TO + static_cast<TexCoord::Type>(x) * (TA - TO)
                                  + static_cast<TexCoord::Type>(y) * (TB - TO));
...
void Mesh::Project(const Camera &camera, DepthMap &depthMap, NormalMap &normalMap) const
{
    ...
    struct RasterMesh : TRasterMesh<RasterMesh>
    {
        ...
        RasterMesh(const Mesh &_mesh, const Camera &_camera, DepthMap &_depthMap, NormalMap &_normalMap)
            : Base(_mesh.vertices, _camera, _depthMap), mesh(_mesh), normalMap(_normalMap),
              R(camera.R)
        {
        }
        ...
        void Raster(const ImageRef &pt, const Point3f &bary)
        {
            ...
            if (depth == Depth(0) || depth > z)
            {
                depth         = z;
                normalMap(pt) = R
                                * normalized(mesh.vertexNormals[idxVerts[0]] * pbary[0]
    ...
    struct RasterMesh : TRasterMesh<RasterMesh>
    {
        ...
        inline bool ProjectVertex(const Mesh::Vertex &pt, int v)
        {
            return (ptc[v] = camera.TransformPointW2C(Cast<REAL>(pt))).z > 0
                   && depthMap
                          .isInsideWithBorder<float, 3>(pti[v] = camera.TransformPointOrthoC2I(ptc[v]));
        }
...
void Mesh::ProjectOrthoTopDown(unsigned resolution, Image8U3 &image, Image8U &mask, Point3 &center) const
{
    ...
    center = camera.TransformPointI2W(Point3(xCenter.x,
                                             xCenter.y,
                                             depthCenter > 0 ? depthCenter : camera.C.z - center.z));
}
...
bool Mesh::Split(FacesChunkArr &chunks, float maxArea)
{
    ...
    struct ChunkInserter
    {
        ...
        void operator()(const Octree::CELL_TYPE &parentCell,
                        Octree::Type parentRadius,
                        const UnsignedArr &children)
        {
    ...
    if (chunks.size() < 2)
        return false;
    DEBUG_EXTRA("Mesh split (%g max-area): %u chunks (%s)",
                maxArea,
                chunks.size(),
                TD_TIMER_GET_FMT().c_str());
    return true;
...
#endif
bool Mesh::TransferTexture(Mesh &mesh,
                           const FaceIdxArr &faceSubsetIndices,
                           unsigned borderSize,
...
// directly transfer the texture onto the new coordinates
    ...
    #pragma omp parallel for schedule(dynamic)
        ...
        FOREACHRAW(idx, num_faces)
        {
            ...
            struct RasterTriangle
            {
                ...
                {
                    return meshTrg.texturesDiffuse[0].size();
                }
                ...
                *this, mesh, mask, faceTexcoords.data() + idxFace * 3, mesh.faceTexindices[idxFace]};
            ...
            Image8U::RasterizeTriangleBary<TexCoord::Type, RasterTriangle, false>(tri[0],
                                                                                  tri[1],
                                                                                  tri[2],
                                                                                  data);
        }
    ...
    else
    {
        ...
        struct IntersectRayMesh
        {
            ...
            IntersectRayMesh(const Mesh &_mesh, const Ray3f &_ray) : mesh(_mesh), ray(_ray)
            {
            ...
            inline void IntersectsRayFace(FIndex idxFace)
            {
                ...
                Type dist;
                if (ray.Intersects<false>(Triangle3f(mesh.vertices[face.x],
    ...
    #pragma omp parallel for schedule(dynamic)
        ...
        FOREACHRAW(idx, num_faces)
        {
            ...
            struct RasterTriangle
            {
                ...
                inline cv::Size Size() const
                {
                    return meshTrg.texturesDiffuse.back().size();
                }
                ...
                const OctreeIntersectRayMesh intRay(octree, meshRef, ray);
                    ...
                    if (intRay.pick.IsValid())
                    {
                        ...
                        const Vertex
                            baryRef(CorrectBarycentricCoordinates(BarycentricCoordinatesUV(meshRef.vertices
                                                                                               [refFace[0]],
                                                                                           meshRef.vertices
                                                                                               [refFace[1]],
                                                                                           meshRef.vertices
