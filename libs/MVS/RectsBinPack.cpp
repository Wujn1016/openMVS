...
MaxRectsBinPack::RectWIdxArr MaxRectsBinPack::Insert(RectWIdxArr &unplacedRects,
...
// 遍历所有未放置的矩形块，在空余空间中找到优的位置，即放置的左下角坐标尽可能小
// ScoreRect评分标准为一个矩阵块，长宽最小的那一边贴着
// 以score1=besty优先，假设第一次放置一个5*5矩阵后，原先的矩阵20*20剩余空余空间为5*15（右上角），15*20下面
// 在这两个空余矩阵中继续放置的时候，矩阵能放置在5*15最好，因为其score1最小，
    ...
    #pragma omp for nowait
            ...
            {
                int score1, score2;
                // score1=besty，score2=bestx,放置的左下角坐标
                Rect newNode(ScoreRect(unplacedRects[i].rect.width,
                                       unplacedRects[i].rect.height,
                                       method,
                                       score1,
                                       score2));
                // 如果得到的评分score小于当前最佳评分privBestScore，更新当前最佳评分
    ...
    #pragma omp critical
            {
                if (privBestScore1 < bestScore1
                    || (privBestScore1 == bestScore1 && privBestScore2 < bestScore2))
                {
...
MaxRectsBinPack::Rect MaxRectsBinPack::ScoreRect(int width,
                                                 int height,
                                                 FreeRectChoiceHeuristic method,
                                                 int &score1,
                                                 int &score2) const
{
    ...
    {
    case RectBestShortSideFit:
        return FindPositionForNewNodeBestShortSideFit(width, height, score1, score2);
    case RectBestLongSideFit:
        return FindPositionForNewNodeBestLongSideFit(width, height, score2, score1);
    case RectBestAreaFit:
        return FindPositionForNewNodeBestAreaFit(width, height, score1, score2);
    case RectBottomLeftRule:
        return FindPositionForNewNodeBottomLeft(width, height, score1, score2);
    case RectContactPointRule:
    {
        Rect newNode = FindPositionForNewNodeContactPoint(width, height, score1);
        score1 = -score1; // Reverse since we are minimizing, but for contact point score bigger is better.
        return newNode;
    }
    default:
        ASSERT("unknown method" == NULL);
        return Rect();
    }
}
...
MaxRectsBinPack::Rect MaxRectsBinPack::FindPositionForNewNodeBottomLeft(int width,
                                                                        int height,
                                                                        int &bestY,
                                                                        int &bestX) const
{
    ...
    for (size_t i = 0; i < freeRectangles.GetSize(); ++i)
    {
        ...
        // 如果需要放置的矩阵宽，高小于空余矩阵的宽高
        // 在该空余矩阵处，贴着y边放置，
        // y最小优先，也就是各个矩阵块能先把x占满最好
...
MaxRectsBinPack::Rect MaxRectsBinPack::FindPositionForNewNodeBestShortSideFit(int width,
                                                                              int height,
...
MaxRectsBinPack::Rect MaxRectsBinPack::FindPositionForNewNodeBestLongSideFit(int width,
                                                                             int height,
                                                                             int &bestShortSideFit,
                                                                             int &bestLongSideFit) const
{
...
MaxRectsBinPack::Rect MaxRectsBinPack::FindPositionForNewNodeBestAreaFit(int width,
                                                                         int height,
                                                                         int &bestAreaFit,
    ...
    for (size_t i = 0; i < usedRectangles.GetSize(); ++i)
    {
        if (usedRectangles[i].x == x + width || usedRectangles[i].x + usedRectangles[i].width == x)
            score += CommonIntervalLength(usedRectangles[i].y,
                                          usedRectangles[i].y + usedRectangles[i].height,
                                          y,
                                          y + height);
        if (usedRectangles[i].y == y + height || usedRectangles[i].y + usedRectangles[i].height == y)
            score += CommonIntervalLength(usedRectangles[i].x,
                                          usedRectangles[i].x + usedRectangles[i].width,
                                          x,
                                          x + width);
    }
    return score;
...
MaxRectsBinPack::Rect MaxRectsBinPack::FindPositionForNewNodeContactPoint(int width,
                                                                          int height,
...
GuillotineBinPack::RectWIdxArr GuillotineBinPack::Insert(RectWIdxArr &unplacedRects,
                                                         bool merge,
                                                         ...
                                                         GuillotineSplitHeuristic splitMethod)
{
    ...
    // Pack rectangles one at a time until we have cleared the rects array of all rectangles or
    // there is no space. unplacedRects will get destroyed in the process.
    RectWIdxArr placedRects;
    while (!unplacedRects.IsEmpty())
    {
        ...
        for (size_t i = 0; i < freeRectangles.size(); ++i)
        {
            for (size_t j = 0; j < unplacedRects.GetSize(); ++j)
            {
                ...
                if (currentRect.width == freeRectangles[i].width
                    && currentRect.height == freeRectangles[i].height)
                {
                    bestFreeRect = i;
                    bestRect     = j;
                    bestFlipped  = false;
                    ...
                    i            = freeRectangles
                            .size(); // Force a jump out of the outer loop as well - we got an instant fit.
                    break;
                }
                // If flipping this rectangle is a perfect match, pick that then.
                else if (currentRect.height == freeRectangles[i].width
                         && currentRect.width == freeRectangles[i].height)
                {
                    bestFreeRect = i;
                    bestRect     = j;
                    bestFlipped  = true;
                ...
                else if (currentRect.width <= freeRectangles[i].width
                         && currentRect.height <= freeRectangles[i].height)
                {
                    int score = ScoreByHeuristic(currentRect.width,
                                                 currentRect.height,
                                                 freeRectangles[i],
                                                 rectChoice);
                    if (score < bestScore)
                ...
                // If not, then perhaps flipping sideways will make it fit?
                         ...
                         && currentRect.width <= freeRectangles[i].height)
                {
                    int score = ScoreByHeuristic(currentRect.height,
                                                 currentRect.width,
                                                 freeRectangles[i],
                                                 rectChoice);
                    if (score < bestScore)
...
GuillotineBinPack::Rect GuillotineBinPack::Insert(int width,
                                                  int height,
                                                  bool merge,
                                                  FreeRectChoiceHeuristic rectChoice,
                                                  GuillotineSplitHeuristic splitMethod)
...
int GuillotineBinPack::ScoreByHeuristic(int width,
                                        int height,
                                        const Rect &freeRect,
                                        FreeRectChoiceHeuristic rectChoice)
{
...
GuillotineBinPack::Rect GuillotineBinPack::FindPositionForNewNode(int width,
                                                                  int height,
                                                                  FreeRectChoiceHeuristic rectChoice,
                                                                  size_t &nodeIndex)
{
...
void GuillotineBinPack::SplitFreeRectByHeuristic(const Rect &freeRect,
...
/// This function will add the two generated rectangles into the freeRectangles array. The caller is
/// expected to remove the original rectangle from the freeRectangles array after that.
...
void GuillotineBinPack::MergeFreeList()
{
    ...
    // Note that we miss any opportunities to merge three rectangles into one. (should call this
    // function again to detect that)
    for (size_t i = 0; i < freeRectangles.size(); ++i)
...
SkylineBinPack::RectWIdxArr SkylineBinPack::Insert(RectWIdxArr &unplacedRects,
...
SkylineBinPack::Rect SkylineBinPack::Insert(int width, int height, LevelChoiceHeuristic method)
{
    if (useWasteMap)
    {
        ...
        Rect node = wasteMap.Insert(width,
                                    height,
                                    true,
...
SkylineBinPack::Rect SkylineBinPack::ScoreRect(int width,
                                               int height,
                                               LevelChoiceHeuristic method,
                                               int &score1,
                                               int &score2,
                                               int &index) const
{
    ...
    {
    case LevelBottomLeft:
        newNode = FindPositionForNewNodeBottomLeft(width, height, score1, score2, index);
        break;
    case LevelMinWasteFit:
        newNode = FindPositionForNewNodeMinWaste(width, height, score2, score1, index);
        break;
    default:
        ASSERT(false);
    }
...
int SkylineBinPack::ComputeWastedArea(int skylineNodeIndex, int width, int height, int y) const
{
    ...
    for (; skylineNodeIndex < (int)skyLine.size() && skyLine[skylineNodeIndex].x < rectRight;
         ++skylineNodeIndex)
    {
...
SkylineBinPack::Rect SkylineBinPack::FindPositionForNewNodeBottomLeft(int width,
                                                                      int height,
                                                                      int &bestHeight,
                                                                      int &bestWidth,
                                                                      int &bestIndex) const
...
SkylineBinPack::Rect SkylineBinPack::FindPositionForNewNodeMinWaste(int width,
                                                                    int height,
                                                                    int &bestHeight,
                                                                    int &bestWastedArea,
                                                                    int &bestIndex) const
