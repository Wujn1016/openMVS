...
public:
    ...
    uint32_t ID; // global ID of the image (the ID given outside the current scene - ex. the index
                 // in the full list of image files)
...
public:
    ...
    inline Image8U::Size GetSize() const
    {
        return Image8U::Size(width, height);
    }
    ...
    unsigned RecomputeMaxResolution(unsigned &level,
                                    unsigned minImageSize,
                                    unsigned maxImageSize = INT_MAX) const;
    ...
    static bool StereoRectifyImages(const Image &image1,
                                    const Image &image2,
    ...
    template <class Archive> void save(Archive &ar, const unsigned int /*version*/) const
    {
        ...
        ar &relName;
        const String relMaskName(maskName.empty() ? String()
                                                  : MAKE_PATH_REL(WORKING_FOLDER_FULL, maskName));
        ar &relMaskName;
        ar &width &height;
        ar &neighbors;
        ar &avgDepth;
