...
bool Image::ReloadImage(unsigned nMaxResolution, bool bLoadPixels)
{
    ...
    IMAGEPTR pImage = ReadImage(name, image);
...
unsigned Image::RecomputeMaxResolution(unsigned &level, unsigned minImageSize, unsigned maxImageSize) const
{
    ...
    return Image8U3::computeMaxResolution(pImage->GetWidth(),
                                          pImage->GetHeight(),
                                          level,
                                          minImageSize,
                                          maxImageSize);
...
REAL Image::ComputeFOV(int dir) const
{
    ...
    case 2: // diagonal
        return 2
               * ATAN(SQRT(REAL(SQUARE(width) + SQUARE(height))) / (camera.K(0, 0) + camera.K(1, 1)));
    }
    return 0;
} // ComputeFOV
...
//  - input leftPoints and rightPoints contains the pairs of corresponding image projections and
//  their depth (optional)
//  - H converts a pixel from original to rectified left-image
//  - Q converts [x' y' disparity 1] in rectified coordinates to [x*z y*z z 1]*w in original image
//  coordinates
//    where disparity=x1-x2
bool Image::StereoRectifyImages(const Image &image1,
                                const Image &image2,
                                const Point3fArr &points1,
                                const Point3fArr &points2,
                                Image8U3 &rectifiedImage1,
                                Image8U3 &rectifiedImage2,
                                Image8U &mask1,
                                Image8U &mask2,
                                Matrix3x3 &H,
                                Matrix4x4 &Q)
{
...
#elif 1
    const REAL t(Camera::StereoRectifyFusiello(image1.GetSize(),
                                               image1.camera,
                                               image2.GetSize(),
                                               image2.camera,
                                               R1,
...
#else
    Pose pose;
    ComputeRelativePose(image1.camera.R,
                        image1.camera.C,
                        image2.camera.R,
                        image2.camera.C,
                        pose.R,
                        pose.C);
    cv::Mat P1, P2;
    cv::stereoRectify(image1.camera.K,
                      cv::noArray(),
                      image2.camera.K,
                      cv::noArray(),
                      image1.GetSize(),
                      pose.R,
                      Vec3(pose.GetTranslation()),
                      R1,
                      R2,
                      P1,
                      P2,
    ...
    // contained in the rectified images
    cv::Size size1(image1.GetSize()), size2(image2.GetSize());
    if (!points1.empty())
        Camera::SetStereoRectificationROI(points1,
                                          size1,
                                          image1.camera,
                                          points2,
                                          size2,
    ...
    const Matrix3x3 H1(K1 * R1 * image1.camera.GetInvK());
    H = H1;
    ...
    cv::Mat(image1.camera.K * R1.t())
        .copyTo(cv::Mat(4, 4, cv::DataType<Matrix4x4::Type>::type, P.val)(cv::Rect(0, 0, 3, 3)));
    Q = P * Q;
    return true;
}
...
template <typename TYPE> float TDisparity2Depth(const Matrix4x4 &Q, const TPoint2<TYPE> &u, float d)
{
...
float Image::Disparity2Distance(const Matrix3x3 &K, const Matrix4x4 &Q, const Point2f &u, float d)
{
    ...
    return (float)(SQRT(SQUARE(pt.x - K(0, 2)) + SQUARE(pt.y - K(1, 2)) + SQUARE(K(0, 0))) * depth
                   / K(0, 0));
}
